...
persistence fails before commit. An absolute `stateFile` remains supported
under a current-user-owned POSIX parent that is not group/other-writable
(including conventional `0755` directories); writable shared parents fail
closed. State bytes and the parent directory are flushed around atomic rename.
...
force-killed after a bounded grace period.
