...
summary-only fallback. If the request is revoked, posting is skipped and state
is left untouched. Read-only reconciliation after an ambiguous or successful
POST remains allowed even though GitHub clears a fulfilled review request.
...
timestamp. Each real poll also checks at most 25 deterministic, rotating
historical records in the selected authenticated account/repository scope and
deletes only exact keys whose pull requests are directly confirmed `CLOSED` or
`MERGED`; search absence, lookup failure, malformed metadata, and HTTP 404 are
never deletion evidence. Deconfigured or unscoped records are eligible only
for the local 365-day expiry, not remote cleanup. The state file is limited to
10,000 review records and 16 MiB. At capacity, an existing record can still be
updated, but a new record must reserve a slot before any external review POST.
There is no arbitrary capacity pruning.
