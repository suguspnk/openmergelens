...
closed. Windows state paths must remain within the private OpenMergeLens home
because Node cannot portably verify an arbitrary parent ACL. State bytes and
the parent directory are flushed around atomic rename.
...
deadline, and an authentication process that ignores graceful termination is
force-killed with its process tree after a bounded grace period even if its
leader exits first.
