...
After a trustworthy non-dry-run search, stale state for requests no longer
returned in that exact account and repository scope is retired. Failed,
incomplete, or inconsistent discovery retains that state and admits no review
candidates. Individual malformed or foreign rows are rejected and suppress
cleanup while other validated requested rows may proceed; dry runs never change
state.
