...
PR returned by a successful, validated search for an active review request,
provided the head commit has not already been reviewed by that identity.
...
The request may be manual or generated by `CODEOWNERS`. After a trustworthy
non-dry-run search, stale state for requests no longer returned in that exact
account and repository scope is retired. Failed or malformed searches retain
that state and never use it as a review candidate; dry runs never change it.
