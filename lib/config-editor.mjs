...
async function editAccounts(currentConfig) {
  ...
  if (p.isCancel(provider) || provider === 'back') return currentConfig;
  ...
  if (existingProviderAccounts.length > 0) {
    ...
    if (p.isCancel(action) || action === 'back') return currentConfig;
    if (action === 'remove') {
      const result = await removeProviderAccounts(currentConfig, provider);
      if (result.reason === 'last-account') {
        p.log.warn('At least one provider account must remain configured.');
      }
      return result.config;
    }
  }
  ...
  const nextConfig = await consentedConfigUpdate(
    currentConfig,
  ...
  try {
    await ensureConfigReviewFiles(providerAccounts, createdFiles);
