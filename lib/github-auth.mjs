...
export function runGitHubAuthCommand(
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (err) => {
      ...
      // If the leader closes after the graceful timeout signal, retain the
      // pending hard-kill timer until it has had a chance to stop descendants.
      // Overflow and completed hard-kill paths own cleanup and cancel it.
      if (!timedOut || outputOverflowed || hardKillStarted) {
    ...
    const onOutput = (streamName, chunk) => {
      if (settled || outputOverflowed || awaitingTreeKill) return;
      ...
      try {
        ...
        // Overflow owns cleanup from this point. Cancel both timers before
        // starting the forced tree termination so a delayed termination
        // cannot race the command timeout or a previously armed hard kill.
        clearTimeout(timeoutHandle);
        clearTimeout(hardKillHandle);
        let termination;
        try {
    ...
    child.once('close', (code, signal) => {
      ...
      if (timedOut) {
        // The graceful timeout path deliberately leaves the forced tree-kill
        // timer authoritative even when the direct child closes first. Do
        // not settle here: a descendant may still be alive, and a failed
        // forced cleanup must remain observable to callers.
        return;
      } else if (code !== 0) {
        finish(Object.assign(
    ...
    timeoutHandle = setTimeout(() => {
      if (settled || awaitingTreeKill || outputOverflowed) return;
      timedOut = true;
      if (platform === 'win32') {
      ...
      hardKillHandle = setTimeout(() => {
        ...
        void Promise.resolve(termination).then(
          () => {
            // Never wait indefinitely for a child that ignores SIGTERM or
            // fails to emit close after a hard tree kill.
            finish(Object.assign(
              new Error(`${command} timed out after ${timeoutMs}ms`),
              { code: 'ETIMEDOUT', signal: 'SIGKILL' },
            ));
...
function sanitizedAuthCommandFailure(normalized, err) {
  ...
  const safeTimeoutCode = typeof err?.timeoutCode === 'string' &&
    ...
    ? err.timeoutCode
    : undefined;
  const safeOverflowCode = typeof err?.overflowCode === 'string' &&
    /^[A-Z][A-Z0-9_]{0,63}$/u.test(err.overflowCode)
    ? err.overflowCode
    : undefined;
  ...
  if (safeOverflowCode !== undefined) cause.overflowCode = safeOverflowCode;
  return Object.assign(
    ...
    ),
    {
      ...
      ...(safeTimeoutCode === undefined ? {} : { timeoutCode: safeTimeoutCode }),
      ...(safeOverflowCode === undefined ? {} : { overflowCode: safeOverflowCode }),
    },
  );
}
