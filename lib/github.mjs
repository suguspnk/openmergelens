...
export function ghSpawn(
  ...
  return new Promise((resolve, reject) => {
    ...
    const terminateWith = (error) => {
      ...
      terminalError = error;
      const rejectTerminationFailure = (cause) => settle(() => reject(Object.assign(
        new Error('gh process tree could not be terminated', { cause }),
        { code: 'ETERMINATE', terminalCode: terminalError.code },
      )));
      if (platform === 'win32') {
        let termination;
        try {
      ...
      void terminate(child, { platform, force: false })
        .catch(rejectTerminationFailure);
...
export async function currentUsername({ auth, timeoutMs } = {}) {
  const out = await gh(['api', 'user', '--jq', '.login'], { auth, timeoutMs });
  return out.trim();
}
...
function appendReviewMarker(body, marker, attribution) {
  return attribution
    ? `${body}\n\n---\n${attribution}\n\n${marker}`
    : `${body}\n\n${marker}`;
}
...
export function prepareReview({
  ...
  const extraBody = unanchorable.length
    ...
    : '';
  const attribution = includeAttribution ? formatAttribution(auth) : null;
  const reviewBody = appendReviewMarker(`${body}${extraBody}`, marker, attribution);
...
export async function postReview({
  ...
  try {
    ...
    const fallbackBody = appendReviewMarker(
      fallbackContent,
      marker,
