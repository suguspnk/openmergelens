...
// Returns a complete array of { repo, number } pairs so every result retains
// the canonical repository slug returned by GitHub for later
// metadata/diff/post calls. The non-enumerable `complete` property lets the
// poller distinguish validated discovery from partial or legacy results while
// preserving the array contract for existing callers.
export async function searchReviewRequestedPRs({ username, repo, auth }) {
  ...
  const fallbackCandidates = fallbackOut
    ...
    .map((numberStr) => {
      ...
      const candidateKey = `${normalizedRepo.toLowerCase()}#${number}`;
      if (fallbackCandidateKeys.has(candidateKey)) {
        throw new Error('GitHub pull request fallback returned a duplicate candidate');
      }
      fallbackCandidateKeys.add(candidateKey);
      return { repo: normalizedRepo, number };
    });
  return completeReviewRequestSearch(fallbackCandidates);
}
