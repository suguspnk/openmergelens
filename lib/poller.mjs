...
async function pollOnceInternal({
  ...
  async function discoverAccount(account) {
    ...
    if (!hasAiProcessingConsent(config)) {
      ...
      return { account, items: [], stateReconciliations: [] };
    }
    let auth;
    try {
    ...
    for (const repo of account.repositories) {
      ...
      if (!searchFailed) {
        ...
        } else if (candidates.complete !== true) {
          await recordFailure(
            {
              subject: repo,
              account: logger.label,
      ...
      if (searchTrustworthy) {
        stateReconciliations.push({
          account,
          repo,
          requestedNumbers: requestedCandidates.map(({ number }) => number),
        });
      }
  ...
  if (!dryRun) {
    for (const { stateReconciliations } of accountQueues) {
      for (const reconciliation of stateReconciliations) {
        const snapshot = normalizeState(state);
        const changed = reconcileRequestedReviewState(state, {
          reviewer: reconciliation.account,
          repo: reconciliation.repo,
          requestedNumbers: reconciliation.requestedNumbers,
        });
        if (!changed) continue;
        try {
          await services.saveState(stateFile, state);
          ...
          await recordFailure(
            {
              subject: reconciliation.repo,
              account: accountLabel(reconciliation.account),
              note: 'review request state cleanup failed',
            },
            accountLabel(reconciliation.account),
            `review request state cleanup failed for ${reconciliation.repo}: ${err.message}`,
            err,
          );
        }
      }
