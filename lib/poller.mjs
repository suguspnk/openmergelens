...
async function pollOnceInternal({
  ...
  function reclaimChoice(
    ...
    const cursorScope = markerMode === 'unproven' ? afterScope : null;
    ...
    for (const [key, entry] of Object.entries(state)) {
      ...
      if (markerMode === 'unproven') {
        const scopeAfterKey = proofAfterKeyForScope?.(scope) ?? null;
        ...
        if (
          scopeAfterKey !== null &&
          key > scopeAfterKey &&
          (scoped.after === null || compareCodeUnits(key, scoped.after.key) < 0)
        ) {
    ...
    for (const [scope, candidatesForScope] of candidates) {
      ...
      const afterCursorScope = markerMode === 'unproven' &&
        cursorScope !== null && scope > cursorScope;
      const preferredProofScope = markerMode === 'unproven' && (
        afterCursorScope !== donor?.afterCursorScope
          ? afterCursorScope
          : compareCodeUnits(scope, donor.scope) < 0
      );
      if (
        donor === null ||
        preferredProofScope ||
        ...
        donor = { scope, score, victim, afterCursorScope };
      }
    }
        ...
        const attempt = await withStateAdmissionQueue(() => withStateWriteQueue(async () => {
          ...
          if (!plan.fits) {
            const proof = reclaimChoice(plan.working, plan.usage, {
              ...
              markerMode: 'unproven',
              afterScope: reviewStateProofAfterScope(state),
              proofAfterKeyForScope: (scope) =>
                reviewStateProofAfterKey(state, scope),
            ...
            try {
              ...
              recordReviewStateProofAfterScope(state, proofScope);
              recordReviewStateProofAfterKey(state, proofScope, proof.key);
              await services.saveState(stateFile, state);
            } catch (err) {
              restoreState(snapshot);
              throw err;
