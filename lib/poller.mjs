...
async function pollOnceInternal({
  ...
  if (legacyEntryLimitExceeded) {
    ...
    function exceedsStateCapacity(candidateState) {
      if (reviewStateEntryCount(candidateState) > MAX_REVIEW_STATE_ENTRIES) {
        return true;
      }
      return serializeState(candidateState, {
        stateFile,
        enforceEntryLimit: false,
        enforceByteLimit: false,
      }).serializedBytes > MAX_STATE_FILE_BYTES;
    }
    function exceedsStateCapacityAfterDeleting(keys) {
      if (keys.length === 0) return exceedsStateCapacity(state);
      const projected = { ...state };
      for (const key of keys) delete projected[key];
      return exceedsStateCapacity(projected);
    }
    ...
    if (exceedsStateCapacity(state)) {
      ...
      if (hasAiProcessingConsent(config)) {
        ...
        const authenticated = await processWithConcurrency(
          migrationAccounts,
          MAX_CONCURRENT_ACCOUNT_DISCOVERIES,
          (account) => authenticateAccount(account, {
        ...
        for (const key of eligibleKeys.slice(0, MAX_LEGACY_STATE_MIGRATION_CHECKS)) {
          ...
          if (trackedPr.state === 'CLOSED' || trackedPr.state === 'MERGED') {
            closedKeys.push(key);
            if (!exceedsStateCapacityAfterDeleting(closedKeys)) break;
          }
        }
      }
    }
    if (exceedsStateCapacityAfterDeleting(closedKeys)) {
