...
async function pollOnceInternal({
  ...
  const { normalizedState: state } = serializeState(
    await services.loadState(stateFile),
    { stateFile },
  );
  let failed = false;
  const accountQueues = [];
  ...
  let markerProofOperationsUsed = 0;
  ...
  function reclaimChoice(
    ...
    for (const [scope, metric] of metrics) {
      ...
      victims.sort((left, right) => {
        const timeDifference = Date.parse(left.entry.lastReviewedAt) -
          Date.parse(right.entry.lastReviewedAt);
        return timeDifference || compareCodeUnits(left.key, right.key);
      });
      let victim = victims[0];
      if (markerMode === 'unproven') {
        const proofVictims = [...victims].sort((left, right) =>
          compareCodeUnits(left.key, right.key));
        victim = afterKey === null
          ? proofVictims[0]
          : proofVictims.find((candidate) => candidate.key > afterKey) ??
            proofVictims[0];
      }
      donors.push({ scope, score, victim });
    }

    donors.sort((left, right) => {
      if (left.score !== right.score) return right.score - left.score;
      return compareCodeUnits(left.scope, right.scope);
    });
    return donors[0]?.victim ?? null;
  }
  ...
  function authenticatedScope(scopeKey) {
    for (const queue of accountQueues) {
      ...
      for (const repo of queue.account.repositories) {
        if (`${reviewer}::${repo.toLowerCase()}` === scopeKey) return queue;
      }
    }
    return null;
  }
  ...
  async function proveReviewMarker(key, entry) {
    ...
    if (!parsed?.reviewer || !scope) return false;
    if (stateMaintenanceOperationsUsed >= MAX_STATE_PROOF_CHECKS_PER_POLL) {
      return false;
    }
    stateMaintenanceOperationsUsed += 1;
    markerProofOperationsUsed += 1;
    ...
    try {
      return await scope.scheduleGitHubOperation(() =>
  ...
  function reserveReviewStateCapacity(reservation) {
    return withStateAdmissionQueue(async () => {
      if (reservedReviewState.has(reservation.key)) {
      ...
      while (true) {
        const attempt = await withStateWriteQueue(async () => {
          for (const [key, originalEntry] of verifiedMarkerEntries) {
          ...
          if (!plan.fits) {
            const proof = reclaimChoice(plan.working, plan.usage, {
              ...
              markerMode: 'unproven',
              afterKey: reviewStateGcAfterKey(state),
            });
            return { proof, usage: plan.usage };
          }
        ...
        if (
          ...
          throw new Error(
            `review state capacity reached at ${attempt.usage.entries} entries ` +
            `and ${attempt.usage.bytes} bytes`,
          );
        }
        attemptedProofs.add(attempt.proof.key);
        const proven = await proveReviewMarker(attempt.proof.key, attempt.proof.entry);
        await persistStateChange(STATE_METADATA_KEY, () => {
          recordReviewStateGcAfterKey(state, attempt.proof.key);
        });
        if (proven) {
  ...
  async function sweepHistoricalReviewState() {
    ...
    const authenticatedScopes = new Map();
    for (const queue of accountQueues) {
      if (!queue.auth || typeof queue.scheduleGitHubOperation !== 'function') continue;
      authenticatedScopes.set(accountKey(queue.account), {
        ...queue,
        repositories: new Set(
          queue.account.repositories.map((repo) => repo.toLowerCase()),
        ),
      });
    }
    ...
    const eligibleKeys = Object.keys(state)
      .filter((key) => {
        ...
        const scope = authenticatedScopes.get(parsed.reviewer.toLowerCase());
        return scope?.repositories.has(parsed.repo.toLowerCase()) === true;
      })
      .sort();
    ...
    const cursorChanged = markerProofOperationsUsed === 0 && cursorNeeded &&
    ...
    try {
      await persistStateBatch(() => {
        ...
        if (markerProofOperationsUsed === 0 && cursorNeeded) {
