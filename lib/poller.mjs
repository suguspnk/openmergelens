...
async function pollOnceInternal({
  ...
  const reservedReviewStateKeys = new Set();
  ...
  function persistReview(key, sha) {
    return persistStateChange(key, () => {
      recordReview(state, key, sha, new Date(services.now()).toISOString());
    });
  }
  ...
  function reserveReviewStateSlot(key) {
    if (state[key] !== undefined) return () => {};
    let occupied = reviewStateEntryCount(state);
    for (const reservedKey of reservedReviewStateKeys) {
      if (state[reservedKey] === undefined) occupied += 1;
    }
    if (occupied >= MAX_REVIEW_STATE_ENTRIES) {
      throw new Error(
        `review state entry limit of ${MAX_REVIEW_STATE_ENTRIES} reached`,
      );
    }
    reservedReviewStateKeys.add(key);
    let released = false;
    return () => {
      if (released) return;
      released = true;
      reservedReviewStateKeys.delete(key);
    };
  }
  ...
  async function sweepHistoricalReviewState() {
    ...
    const checkedKeys = rotatedKeys.slice(0, MAX_STATE_GC_CHECKS_PER_POLL);
    const closedKeys = [];
    ...
    const cursorNeeded = eligibleKeys.length > MAX_STATE_GC_CHECKS_PER_POLL;
  ...
  async function runReviewCandidate({
    ...
    let releaseStateReservation;
    try {
      releaseStateReservation = reserveReviewStateSlot(key);
    } catch (err) {
      return recordFailure(
        { ...prEntry, note: 'review state capacity reached' },
        logger.label,
        `cannot post ${repo}#${number}: ${err.message}`,
        err,
      );
    }
