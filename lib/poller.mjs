...
async function pollOnceInternal({
  ...
  function reclaimChoice(
    ...
    for (const [scope, metric] of metrics) {
      ...
      donors.push({ scope, score, victim: victims[0] });
    }
  ...
  async function proveReviewMarker(key, entry) {
    ...
    if (stateMaintenanceOperationsUsed >= MAX_STATE_GC_CHECKS_PER_POLL) {
      return false;
    }
  ...
  function reserveReviewStateCapacity(reservation) {
    return withStateAdmissionQueue(async () => {
      ...
      while (true) {
        ...
        if (
          !attempt.proof ||
          stateMaintenanceOperationsUsed >= MAX_STATE_GC_CHECKS_PER_POLL
        ) {
          throw new Error(
        ...
        if (await proveReviewMarker(attempt.proof.key, attempt.proof.entry)) {
  ...
  async function sweepHistoricalReviewState() {
    ...
    try {
      await persistStateBatch(() => {
        ...
        if (cursorNeeded) recordReviewStateGcAfterKey(state, nextAfterKey);
      });
