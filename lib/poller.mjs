  ...
  async function discoverAccount(account) {
    ...
    for (const repo of account.repositories) {
      ...
      if (
        ...
        try {
          await persistStateChange(STATE_METADATA_KEY, () =>
            reconcileBitbucketPostingPlans(state, {
              account,
              repo,
              requestedNumbers: new Set(requestedCandidates.map(({ number }) => number)),
              observedAt: new Date(services.now()).toISOString(),
            }));
        } catch (err) {
          await recordFailure(
            { subject: repo, account: logger.label, note: 'posting plan reconciliation failed' },
            logger.label,
            `posting plan reconciliation failed for ${repo}: ${err.message}`,
            err,
          );
        }
      }
      ...
      const candidateGroups = [
        ...
        ...(provider === 'bitbucket'
          ? [{ source: 'tracked', items: bitbucketTrackedCandidatesForRepository(state, account, repo) }]
          : []),
      ];
