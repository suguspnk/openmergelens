...
async function pollOnceInternal({
  ...
  const { normalizedState: state } = serializeState(
    loadedState,
    { stateFile, enforceEntryLimit: !legacyEntryLimitExceeded },
  );
  let failed = false;
  const accountQueues = [];
  ...
  if (legacyEntryLimitExceeded) {
    ...
    const closedKeys = [];
    const attemptedKeys = [];
    if (reviewStateEntryCount(state) > MAX_REVIEW_STATE_ENTRIES) {
      ...
      if (hasAiProcessingConsent(config)) {
        ...
        for (const key of eligibleKeys.slice(0, MAX_LEGACY_STATE_MIGRATION_CHECKS)) {
          ...
          if (pullRequestMetadataError(trackedPr)) continue;
          if (trackedPr.state === 'CLOSED' || trackedPr.state === 'MERGED') {
            closedKeys.push(key);
            if (
              reviewStateEntryCount(state) - closedKeys.length <=
              MAX_REVIEW_STATE_ENTRIES
            ) break;
          }
        }
