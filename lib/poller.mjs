...
const MAX_LEGACY_STATE_MIGRATION_CHECKS = 1_000;
const MAX_LEGACY_STATE_MIGRATION_FAILURES = 3;
const MAX_LEGACY_STATE_MIGRATION_DURATION_MS = 15_000;
const MAX_LEGACY_STATE_MIGRATION_REQUEST_MS = 5_000;
...
function pullRequestMetadataError(pr) {
  ...
  if (!GITHUB_PULL_REQUEST_STATES.has(pr.state)) {
    return 'expected OPEN, CLOSED, or MERGED state';
  }
...
function bitbucketTrackedCandidatesForRepository(state, account, repo) {
  const reviewer = accountKey(account);
  const normalizedRepo = repo.toLowerCase();
  const candidates = [];
  for (const [key, entry] of Object.entries(state)) {
    if (key === STATE_METADATA_KEY || bitbucketTerminalHeadFor(state, key, entry?.bitbucketTerminalHeadSha)) continue;
    ...
    if (
      ...
      candidates.push({ repo, number: parsed.number, source: 'tracked' });
    }
  }
  return candidates;
}
...
async function pollOnceInternal({
  ...
  const services = {
    resolveGitHubAuth,
    resolveBitbucketAuth: (account) => resolveBitbucketAuth(account, {
    ...
    loadState: (statePath) => loadState(statePath, {
      hardenPermissions: !dryRun,
      allowEntryLimitMigration: true,
    }),
    saveState: (statePath, nextState, saveOptions = {}) => saveState(
      statePath,
      nextState,
      {
        ...saveOptions,
        onPostCommitError: async (err) => pollLogger.warn(
          `review state committed, but directory durability could not be confirmed: ${err.message}`,
          {
            event: 'state.commit-durability-warning',
            fields: { scope: 'review state' },
            error: err,
          },
        ),
      },
  ...
  // Route the revalidation through the injectable metadata reader. This keeps
  // the mutation boundary testable and ensures the default path uses the same
  // authenticated Bitbucket client as normal metadata reads.
  if (!Object.prototype.hasOwnProperty.call(dependencies, 'hasActiveBitbucketReviewRequest')) {
    services.hasActiveBitbucketReviewRequest = Object.prototype.hasOwnProperty.call(
      dependencies,
      'getBitbucketPullRequest',
    )
      ? async ({ repo, number, account, auth }) => {
        const pullRequest = await services.getBitbucketPullRequest({ repo, number, auth });
        // Test and adapter seams that do not expose reviewer IDs preserve the
        // historical metadata contract. The built-in client always exposes a
        // verified list and therefore rejects a revoked request.
        return !Array.isArray(pullRequest?.reviewerIds) ||
          pullRequest.reviewerIds.includes(account.accountId.toLowerCase());
      }
      : hasActiveBitbucketReviewRequest;
  }
  const configuredReviewScopes = configuredReviewScopeUniverse(config);
  const accounts = selectConfiguredAccounts([
    ...(config.githubAccounts || []),
    ...(config.bitbucketAccounts || []),
  ], accountSelector);
  ...
  const legacyEntryLimitExceeded =
    reviewStateNeedsEntryLimitMigration(loadedState);
  ...
  const { normalizedState: state } = serializeState(
    ...
    {
      stateFile,
      enforceEntryLimit: !legacyEntryLimitExceeded,
      enforceByteLimit: !legacyEntryLimitExceeded,
    },
  );
  let failed = false;
  const accountQueues = [];
  ...
  let stateWriteReconciliationFailure;
  ...
  if (accounts.some((account) => accountProvider(account) === 'bitbucket')) {
    try {
      const preview = cloneReviewState(state);
      const previewRetention = pruneBitbucketTerminalHeads(
        preview,
        new Date(services.now()).toISOString(),
      );
      if (previewRetention.changed) {
        const prune = () => {
          pruneBitbucketTerminalHeads(
            state,
            new Date(services.now()).toISOString(),
        ...
        if (dryRun) prune();
        else await persistStateBatch(prune);
      }
    } catch (err) {
      await recordFailure(
        { subject: 'review state', note: 'terminal-head retention failed' },
        'bitbucket.org',
        `Bitbucket terminal-head retention failed: ${err.message}`,
        err,
      );
    }
  }
  ...
  async function authenticateAccount(
    ...
    try {
      if (accountProvider(account) === 'bitbucket') {
        const auth = await services.resolveBitbucketAuth(account);
        return { account, auth, scheduleGitHubOperation };
      }
      const remainingMs = deadline === undefined
        ? undefined
        : deadline - services.monotonicNow();
      if (remainingMs !== undefined && remainingMs <= 0) {
        throw new Error('authentication exceeded the legacy migration deadline');
      }
      const timeoutMs = remainingMs === undefined
        ? undefined
        : Math.max(1, Math.min(maxRequestMs, Math.ceil(remainingMs)));
      const auth = await services.resolveGitHubAuth(account, { timeoutMs });
      const usernameRemainingMs = deadline === undefined
        ? undefined
        : deadline - services.monotonicNow();
      if (usernameRemainingMs !== undefined && usernameRemainingMs <= 0) {
        throw new Error('authentication exceeded the legacy migration deadline');
      }
      const usernameTimeoutMs = usernameRemainingMs === undefined
        ? undefined
        : Math.max(1, Math.min(maxRequestMs, Math.ceil(usernameRemainingMs)));
      const authenticatedUsername = await scheduleGitHubOperation(
        () => services.currentUsername({ auth, timeoutMs: usernameTimeoutMs }),
        usernameTimeoutMs === undefined
          ? undefined
          : { signal: AbortSignal.timeout(usernameTimeoutMs) },
      );
      ...
      return { account, auth: null, scheduleGitHubOperation: null };
    }
  }

  if (legacyEntryLimitExceeded) {
    const repairDeadline = services.monotonicNow() +
      MAX_LEGACY_STATE_MIGRATION_DURATION_MS;
    const snapshot = normalizeState(state, { enforceEntryLimit: false });
      ...
      await recordFailure(
        {
          ...
          note: 'legacy state capacity migration required',
        },
        'review state',
        'legacy over-cap review state requires a non-dry poll for atomic migration',
      );
      return { failed, reviewed: 0, outcomes: [], failures };
    }
    // Resolve a one-account legacy namespace before remote repair so unscoped
    // records participate in authenticated closure checks. Existing scoped
    // records retain precedence on collision, and the startup snapshot still
    // covers adoption for every failure path below.
    if (config.githubAccounts.length === 1) {
      migrateLegacyStateForReviewer(state, config.githubAccounts[0]);
    }
    let projectedEntryCount = reviewStateEntryCount(state);
    let projectedSerializedBytes = serializeState(state, {
      stateFile,
      enforceEntryLimit: false,
      enforceByteLimit: false,
    }).serializedBytes;
    function projectedStateExceedsCapacity() {
      return projectedEntryCount > MAX_REVIEW_STATE_ENTRIES ||
        projectedSerializedBytes > MAX_STATE_FILE_BYTES;
    }
    function projectClosedKey(key) {
      // In the pretty-printed top-level object, removing one property removes
      // its indented property block plus exactly one comma/newline separator.
      // State is already canonicalized, so this byte delta is exact without
      // cloning and reserializing the full (potentially 32 MiB) object.
      const property = JSON.stringify({ [key]: state[key] }, null, 2);
      const propertyBlock = property.slice(2, -2);
      projectedEntryCount -= 1;
      projectedSerializedBytes -=
        Buffer.byteLength(propertyBlock, 'utf8') + Buffer.byteLength(',\n');
    }
    const closedKeys = [];
    const attemptedKeys = [];
    if (projectedStateExceedsCapacity()) {
      // The repair's GitHub reads share discovery's config-wide consent
      // boundary. Local validation and deterministic expiry above remain safe.
      if (hasAiProcessingConsent(config)) {
        const migratableReviewers = new Set(
          Object.keys(state)
            .map((key) => parsePrKey(key)?.reviewer)
            .filter(Boolean),
        );
        const migrationAccounts = accounts.filter((account) =>
          migratableReviewers.has(accountKey(account)),
        );
        const reviewerPosition = new Map();
        for (const [position, key] of Object.keys(state).entries()) {
          const reviewer = parsePrKey(key)?.reviewer;
          if (reviewer && !reviewerPosition.has(reviewer)) {
            reviewerPosition.set(reviewer, position);
          }
        }
        migrationAccounts.sort((left, right) =>
          reviewerPosition.get(accountKey(left)) -
            reviewerPosition.get(accountKey(right)),
        );
        const authenticated = [];
        // Schedule bounded authentication batches only while the end-to-end
        // migration deadline remains live. A slow first batch therefore
        // cannot cause thousands of already-queued username requests to run
        // after the repair window has expired.
        let offset = 0;
        while (
          offset < migrationAccounts.length &&
          services.monotonicNow() < repairDeadline
        ) {
          const batch = migrationAccounts.slice(
            offset,
            offset + MAX_CONCURRENT_ACCOUNT_DISCOVERIES,
          );
          const attemptedReviewers = new Set(batch.map(accountKey));
          const batchResults = await Promise.all(
            batch
              .map((account) => authenticateAccount(account, {
                deadline: repairDeadline,
                maxRequestMs: MAX_LEGACY_STATE_MIGRATION_REQUEST_MS,
              })),
          );
          authenticated.push(...batchResults);
          offset += MAX_CONCURRENT_ACCOUNT_DISCOVERIES;
          // This is the single sample used to decide the next scheduling
          // gate. If it closes the loop, persist this completed batch's
          // rotation now rather than sampling again in the loop condition and
          // losing fairness at the deadline boundary.
          const batchCompletedAt = services.monotonicNow();
          const nextBatchBlockedByDeadline =
            offset < migrationAccounts.length &&
            batchCompletedAt >= repairDeadline;
          if (nextBatchBlockedByDeadline) {
            // Persist authentication-window fairness using the existing
            // byte-neutral entry order only when this batch consumes the
            // repair deadline. Ordinary successful batches retain the
            // existing per-entry closure rotation behavior.
            // Append iteratively: predecessor states may legitimately contain
            // well over 100,000 compact entries, which exceeds JavaScript's
            // bounded function-argument list when spread into push().
            for (const key of Object.keys(state)) {
              if (attemptedReviewers.has(parsePrKey(key)?.reviewer)) {
        ...
        for (const queue of authenticated) {
          ...
          for (const repo of queue.account.repositories) {
            authenticatedReviewScopes.set(`${reviewer}::${repo.toLowerCase()}`, queue);
          }
        }

        const eligibleKeys = Object.keys(state)
          .filter((key) => authenticatedScope(reviewScopeKey(key)) !== null);
        let repairFailures = 0;
        for (const key of eligibleKeys.slice(0, MAX_LEGACY_STATE_MIGRATION_CHECKS)) {
          const remainingMs = repairDeadline - services.monotonicNow();
          ...
          try {
            const operationTimeoutMs = Math.max(
              1,
              Math.min(
                MAX_LEGACY_STATE_MIGRATION_REQUEST_MS,
            ...
            repairFailures += 1;
            if (repairFailures >= MAX_LEGACY_STATE_MIGRATION_FAILURES) break;
            continue;
          }
          if (pullRequestMetadataError(trackedPr)) {
            repairFailures += 1;
            if (repairFailures >= MAX_LEGACY_STATE_MIGRATION_FAILURES) break;
            continue;
          }
          if (trackedPr.state === 'CLOSED' || trackedPr.state === 'MERGED') {
            closedKeys.push(key);
            projectClosedKey(key);
            if (!projectedStateExceedsCapacity()) break;
          }
        }
      }
    }
    if (projectedStateExceedsCapacity()) {
      if (attemptedKeys.length > 0) {
        try {
          ...
          recordReviewStateMigrationWindow(state, attemptedKeys);
          await services.saveState(stateFile, state, {
            allowEntryLimitMigration: true,
          });
          if (expiredKeys.length + closedKeys.length > 0) {
            pollLogger.info(
              `advanced legacy over-cap review state by retiring ` +
              `${expiredKeys.length + closedKeys.length} entries`,
            );
          }
        } catch (err) {
          err = await reconcileStateAfterSaveFailure(snapshot, err);
          await recordFailure(
            {
              subject: 'review state',
              note: 'legacy state capacity migration failed',
            },
            'review state',
            `legacy review state migration progress failed: ${err.message}`,
            err,
          );
          return { failed, reviewed: 0, outcomes: [], failures };
        }
      ...
      await recordFailure(
        ...
        'review state',
        `legacy review state remains above ${MAX_REVIEW_STATE_ENTRIES} entries ` +
          'after bounded retention and closure cleanup',
      );
      return { failed, reviewed: 0, outcomes: [], failures };
    }
    try {
      ...
      await services.saveState(stateFile, state);
      pollLogger.info(
        `migrated legacy over-cap review state by retiring ` +
        `${expiredKeys.length + closedKeys.length} entries`,
      );
    } catch (err) {
      err = await reconcileStateAfterSaveFailure(snapshot, err);
      await recordFailure(
        ...
        'review state',
        `legacy review state capacity migration failed: ${err.message}`,
        err,
      );
      return { failed, reviewed: 0, outcomes: [], failures };
    }
  ...
  if (experimentalProofMetadataPresent && !legacyEntryLimitExceeded && !dryRun) {
    try {
      await services.saveState(stateFile, state);
    } catch (err) {
      await recordFailure(
  ...
  async function reconcileStateAfterSaveFailure(snapshot, err) {
    if (err?.code !== REVIEW_STATE_COMMIT_INDETERMINATE) {
      restoreState(snapshot);
      return err;
    }
    try {
      const reloaded = await services.loadState(stateFile);
      const migrationNeeded = reviewStateNeedsEntryLimitMigration(reloaded);
      const { normalizedState } = serializeState(reloaded, {
        stateFile,
        enforceEntryLimit: !migrationNeeded,
        enforceByteLimit: !migrationNeeded,
      });
      restoreState(normalizedState);
      return err;
    } catch (reloadError) {
      stateWriteReconciliationFailure ??= Object.assign(
        new AggregateError(
          [err, reloadError],
          'review state commit became indeterminate and strict reload failed; ' +
            'subsequent state writes are disabled for this poll',
        ),
        { code: 'ESTATEWRITESDISABLED' },
      );
      return stateWriteReconciliationFailure;
    }
  }

  function withStateWriteQueue(operation) {
    const write = stateWriteQueue.then(() => {
      if (stateWriteReconciliationFailure) {
        throw stateWriteReconciliationFailure;
      }
      return operation();
    });
  ...
  function persistStateBatch(change) {
    return withStateWriteQueue(async () => {
      ...
      const result = change();
      if (result?.changed === false || result === false) return result;
      try {
        assertProjectedCapacity();
        await services.saveState(stateFile, state);
      } catch (err) {
        throw await reconcileStateAfterSaveFailure(snapshot, err);
      }
    });
  }
  ...
  function persistReview(reservation, { clearBitbucketPlan = false } = {}) {
    return persistStateChange(reservation.key, () => {
      ...
      if (clearBitbucketPlan) clearBitbucketPostingPlan(state, reservation.key);
    });
  }

  function retireTrackedState(key) {
    if (state[key] === undefined && !hasBitbucketPostingPlan(state, key)) return Promise.resolve();
    return persistStateChange(key, () => {
      delete state[key];
      clearBitbucketPostingPlan(state, key);
    });
  }
  ...
  if (config.githubAccounts.length === 1 && !legacyEntryLimitExceeded) {
  ...
  if (!legacyEntryLimitExceeded && !dryRun) {
    const snapshot = normalizeState(state);
    const expiredKeys = expireReviewState(state, services.now());
  ...
  async function discoverAccount(account) {
    ...
    const priorScope = `${accountKey(account)}::${account.repositories[0].toLowerCase()}`;
    const authenticated = authenticatedReviewScopes.get(priorScope) ??
      await authenticateAccount(account);
    if (!authenticated.auth) {
      return { account, items: [] };
    }
    const { auth, scheduleGitHubOperation } = authenticated;
    ...
    for (const repo of account.repositories) {
      ...
      try {
        candidates = await timedStep(
          ...
          () => scheduleGitHubOperation(() =>
            provider === 'bitbucket'
              ? services.searchBitbucketReviewRequestedPRs({ account, repo, auth })
      ...
      if (!searchFailed) {
        ...
        } else if (provider === 'github' && !hasTrustedSearchProvenance(
          candidates,
      ...
      const trackedBitbucketCandidates = provider === 'bitbucket'
        ? bitbucketTrackedCandidatesForRepository(state, account, repo)
        : [];

      if (
        provider === 'bitbucket' && searchTrustworthy && !dryRun &&
        (
          state[STATE_METADATA_KEY]?.bitbucketPostingPlans ||
          trackedBitbucketCandidates.length > 0
        )
      ) {
        try {
          const observedAt = new Date(services.now()).toISOString();
          const requestedNumbers = new Set(requestedCandidates.map(({ number }) => number));
          await persistStateChange(STATE_METADATA_KEY, () => {
            const requestReconciliation = reconcileBitbucketReviewRequests(state, {
              account,
              repo,
              requestedNumbers,
              observedAt,
            });
            const postingPlanReconciliation = reconcileBitbucketPostingPlans(state, {
              account,
              repo,
              requestedNumbers,
              observedAt,
            ...
            return {
              changed: requestReconciliation.changed || postingPlanReconciliation.changed,
            };
          });
        } catch (err) {
          await recordFailure(
            { subject: repo, account: logger.label, note: 'Bitbucket state reconciliation failed' },
            logger.label,
            `Bitbucket state reconciliation failed for ${repo}: ${err.message}`,
            err,
          );
        }
      }
      ...
      const candidateGroups = [
        { source: 'requested', items: requestedCandidates },
        ...(provider === 'bitbucket'
          ? [{ source: 'tracked', items: trackedBitbucketCandidates }]
          : []),
      ];
      candidates = candidateGroups.flatMap(({ source, items: group }) => {
        const rotated = rotateCandidateGroup(group, { account, repo, source, state });
        if (rotated.length > 0) {
          candidateCursorPlans.set(rotated[0].candidateCursor.key, rotated[0].candidateCursor);
        }
        return rotated;
      });
      ...
      for (const candidate of candidates) {
        ...
        items.push({ ...candidate, auth, provider, scheduleGitHubOperation });
      }
    }
  ...
  async function sweepHistoricalReviewState() {
    ...
    const eligibleKeys = Object.keys(state)
      .filter((key) => {
        ...
        const scope = authenticatedScope(reviewScopeKey(key));
        return scope !== null && accountProvider(scope.account) === 'github';
      })
      .sort();
    ...
    if (cursorNeeded) {
      ...
      const remainingKeys = rotatedKeys.filter((key) => !closed.has(key));
      const nextUncheckedKey = rotatedKeys[checkedKeys.length];
      const nextIndex = remainingKeys.indexOf(nextUncheckedKey);
      cursorAnchor = remainingKeys[
        (nextIndex - 1 + remainingKeys.length) % remainingKeys.length
      ];
    }
    const cursorChanged = cursorNeeded &&
      cursorAnchor !== reviewStateGcAfterKey(state);
    if (closedKeys.length === 0 && !cursorChanged) return;
    try {
      await persistStateBatch(() => {
        for (const key of closedKeys) delete state[key];
        if (cursorNeeded) {
          recordReviewStateGcPosition(state, cursorAnchor);
        }
      });
  ...
  async function runReviewCandidate({
    ...
    try {
      pr = await timedStep(
        ...
        () => scheduleGitHubOperation(() =>
          provider === 'bitbucket'
            ? services.getBitbucketPullRequest({ repo, number, auth })
    ...
    if (pr.state !== 'OPEN') {
      if (
        ...
        (state[key] !== undefined || hasBitbucketPostingPlan(state, key))
      ) {
        try {
    ...
    if (provider === 'bitbucket' && bitbucketTerminalHeadFor(state, key, pr.headRefOid)) {
      logger.info(`skip ${repo}#${number} (partial posting plan expired for this head)`);
      return null;
    }
    if (provider === 'bitbucket' && source === 'requested' && state[key]?.lastReviewedSha) {
      const requestWasRemoved = state[key].bitbucketRequestRemovedAt !== undefined;
      const hasPreparedPostingPlan = bitbucketPostingPlanFor(
        state,
        key,
        pr.headRefOid,
      ) !== null;
      if (state[key].lastReviewedSha === pr.headRefOid && requestWasRemoved && !dryRun) {
        try {
          await persistStateChange(key, () => clearBitbucketReviewRequestRemoval(state, key));
        } catch (err) {
          return recordFailure(
            { ...baseEntry, note: 'review request reconciliation failed' },
            logger.label,
            `review request reconciliation failed for ${repo}#${number}: ${err.message}`,
            err,
          );
        }
      } else if (
        state[key].lastReviewedSha !== pr.headRefOid &&
        !requestWasRemoved &&
        !hasPreparedPostingPlan
      ) {
        logger.info(`skip ${repo}#${number} (fresh Bitbucket review request required)`);
        return null;
      }
    }
    ...
    // An open Bitbucket PR found only in tracked state is no longer an active
    // review request. Reconcile a previously posted marker if one exists, but
    // do not reserve the finite actionable-review quota or run the reviewer.
    // Otherwise stale tracked entries could crowd out requested reviews from
    // other configured repositories in the same poll.
    if (provider === 'bitbucket' && source === 'tracked') {
      const reviewMarker = services.createBitbucketReviewMarker({
        account,
        repo,
        number,
      ...
      try {
        alreadyPosted = await scheduleGitHubOperation(() =>
          services.bitbucketReviewAlreadyPosted({
            repo,
            number,
            marker: reviewMarker,
            auth,
      ...
      if (!alreadyPosted) {
        logger.info(`skip ${repo}#${number} (Bitbucket reviewer is not currently requested)`);
        return null;
      }
      if (dryRun) {
        logger.info(
    ...
    try {
    const reviewMarker = (provider === 'bitbucket'
      ? services.createBitbucketReviewMarker
      : services.createReviewMarker)({
      account,
      repo,
      number,
      commitId: pr.headRefOid,
    ...
    if (alreadyPosted) {
      ...
      try {
        await persistReview(reservation, {
          clearBitbucketPlan: provider === 'bitbucket',
        });
        logger.info(`reconciled existing review for ${repo}#${number}`);
        return { status: 'recovered', ...prEntry };
      } catch (err) {
    ...
    try {
      diff = await timedStep(
        ...
        () => scheduleGitHubOperation(() =>
          provider === 'bitbucket'
            ? services.getBitbucketPullRequestDiff({ repo, number, auth })
    ...
    let postingPlan = provider === 'bitbucket'
      ? bitbucketPostingPlanFor(state, key, pr.headRefOid)
      : null;
    let review = postingPlan
      ? { summary: postingPlan.body, findings: postingPlan.comments }
      : null;
    if (postingPlan) logger.info(`resuming prepared review for ${repo}#${number}`);
    if (!postingPlan) {
    let template;
    let learnings;
    try {
      ...
      review = await timedStep(
        ...
        () => {
          ...
          return services.invokeMultiPassReview({
            ...
            environment: provider === 'github'
              ? buildReviewerEnvironment(reviewerCommand, authEnvironment(auth))
              : withoutGitHubCredentials(buildReviewerEnvironment(
                reviewerCommand,
                services.reviewerSourceEnvironment,
              )),
            githubAccess: provider === 'github' ? {
              repo,
              number,
            ...
            providerDiff: provider === 'bitbucket' ? diff : undefined,
      ...
      try {
        reviewRequestActive = await scheduleGitHubOperation(() =>
          services.hasActiveReviewRequest({
            repo, number, username: account.username, auth,
          }),
        );
      } catch (err) {
        return recordFailure(
    ...
    if (dryRun) {
      let prepared;
      try {
        prepared = (provider === 'bitbucket'
          ? services.prepareBitbucketReview
          : services.prepareReview)({
          commitId: pr.headRefOid,
          body: review.summary,
          ...
          marker: reviewMarker,
          auth,
          includeAttribution: reviewAttributionEnabled(config, account, repo),
        });
      } catch (err) {
        return recordFailure(
    ...
    if (provider === 'bitbucket' && !postingPlan) {
      try {
        // Bitbucket can reject an inline anchor after it accepted the plan.
        // Validate the all-summary fallback before making a retry plan durable.
        services.prepareBitbucketReview({
          commitId: pr.headRefOid,
          body: review.summary,
          comments: review.findings,
        ...
        await persistStateChange(STATE_METADATA_KEY, () => {
          postingPlan = recordBitbucketPostingPlan(state, key, {
            commitId: pr.headRefOid,
            body: review.summary,
            comments: review.findings,
          });
        });
        review = { summary: postingPlan.body, findings: postingPlan.comments };
      } catch (err) {
        return recordFailure(
          { ...prEntry, note: 'posting plan persistence failed' },
          logger.label,
          `posting plan persistence failed for ${repo}#${number}: ${err.message}`,
          err,
        );
      }
    }
          ...
          () => (provider === 'bitbucket'
            ...
            scheduleMutation: (
              ...
              const validateMutationBoundary = async () => {
                const mutationPr = provider === 'bitbucket'
                  ? await services.getBitbucketPullRequest({ repo, number, auth })
                  : await services.getPullRequest({ repo, number, auth });
                ...
                const stillRequested = provider === 'bitbucket'
                  ? await services.hasActiveBitbucketReviewRequest({
                    repo, number, account, auth,
                  })
                  : await services.hasActiveReviewRequest({
                    repo, number, username: account.username, auth,
                  });
                if (!stillRequested) {
                  logger.info(
