...
async function pollOnceInternal({
  ...
  function reclaimChoice(
    ...
    const cursorScope = markerMode === 'unproven' && afterKey !== null
      ? reviewScopeKey(afterKey)
      : null;
    const cursorPosition = cursorScope === null
      ? null
      : { scope: cursorScope, localKey: afterKey.slice(cursorScope.length) };
    const compareProofPositions = (left, right) =>
      compareCodeUnits(left.localKey, right.localKey) ||
    ...
    for (const [key, entry] of Object.entries(state)) {
      ...
      const candidate = {
        ...
        entry,
        bytes,
        position: { scope, localKey: key.slice(scope.length) },
      };
      let scoped = candidates.get(scope);
      if (!scoped) {
        scoped = { first: null, after: null, oldest: null };
        candidates.set(scope, scoped);
      }
      if (markerMode === 'unproven') {
        if (scoped.first === null || compareCodeUnits(key, scoped.first.key) < 0) {
          scoped.first = candidate;
        }
        if (
          cursorPosition !== null &&
          compareProofPositions(candidate.position, cursorPosition) > 0 &&
          (
            scoped.after === null ||
            compareProofPositions(candidate.position, scoped.after.position) < 0
          )
        ) {
          scoped.after = candidate;
        }
      } else {
    ...
    for (const [scope, candidatesForScope] of candidates) {
      ...
      const victim = markerMode === 'unproven'
        ? candidatesForScope.after ?? candidatesForScope.first
        : candidatesForScope.oldest;
      const afterCursor = markerMode === 'unproven' &&
        candidatesForScope.after !== null;
      const preferredAtEqualScore = markerMode === 'unproven'
        ? (
            afterCursor !== donor?.afterCursor
              ? afterCursor
              : compareProofPositions(victim.position, donor.victim.position) < 0
          )
        : donor !== null && compareCodeUnits(scope, donor.scope) < 0;
      if (
        donor === null ||
        score > donor.score ||
        (score === donor.score && preferredAtEqualScore)
      ) {
        donor = { scope, score, victim, afterCursor };
      }
    }
  ...
  function reserveReviewStateCapacity(reservation) {
    return withStateAdmissionQueue(async () => {
      ...
      while (true) {
        ...
        if (proofResult.remotelyChecked) {
