...
async function pollOnceInternal({
  ...
  if (legacyEntryLimitExceeded) {
    ...
    try {
      ...
      // A successful over-cap repair is also the only startup migration save
      // in this poll. Adopt unambiguous legacy keys before that save so
      // discovery observes their reviewed SHA/marker immediately instead of
      // risking a duplicate review before the next poll.
