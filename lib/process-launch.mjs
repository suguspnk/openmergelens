...
export function terminateProcessTree(
  ...
  if (!child || !Number.isInteger(child.pid)) {
    try {
      const result = child?.kill?.(force ? 'SIGKILL' : 'SIGTERM');
      if (result === false) {
        return Promise.reject(Object.assign(
          new Error('failed to signal process without a verified PID'),
          { code: 'ETERMINATE' },
        ));
      }
    } catch {
  ...
  if (platform === 'win32') {
    if (!force) {
      try {
        ...
        if (result === false) {
          // Node reports false when the child has already exited in a small
          // race between the timeout path and the kill call. Treat that as a
          // successful graceful stop only when the child object confirms the
          // terminal state; a false signal with no exit evidence must remain
          // an explicit termination failure.
          if (childHasExited(child)) {
            return Promise.resolve();
          }
          return Promise.reject(Object.assign(
            new Error(`failed to terminate Windows process ${child.pid}`),
            { code: 'ETERMINATE', pid: child.pid },
          ));
        }
      } catch {
        if (childHasExited(child)) {
  ...
  try {
    const groupSignalResult = process.kill(-child.pid, force ? 'SIGKILL' : 'SIGTERM');
    if (groupSignalResult === false) {
      throw Object.assign(
        new Error(`failed to signal POSIX process group ${child.pid}`),
        { code: 'ESRCH' },
      );
    }
    return Promise.resolve();
    ...
    try {
      const leaderSignalResult = child.kill(force ? 'SIGKILL' : 'SIGTERM');
      if (leaderSignalResult === false) {
        throw Object.assign(
          new Error(`failed to signal POSIX process ${child.pid}`),
          { code: 'ESRCH' },
        );
      }
      return Promise.resolve();
      ...
      return Promise.reject(Object.assign(
        new Error(
          `failed to terminate POSIX process tree ${child.pid}`,
          { cause: groupCause },
        ),
        {
          code: 'ETERMINATE',
