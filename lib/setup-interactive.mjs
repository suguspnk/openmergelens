...
export async function configureBitbucketAccounts({
  ...
  let addAnother = selected.includes(ADD_BITBUCKET_ACCOUNT);
  while (addAnother) {
    ...
    const nextAction = await prompts.select({
      message: 'Add another Bitbucket Cloud account?',
      options: [
        { value: 'done', label: 'Done adding accounts' },
        { value: 'add', label: 'Add another account…' },
      ],
      initialValue: 'done',
    });
    if (prompts.isCancel(nextAction)) onCancel();
    if (!['add', 'done'].includes(nextAction)) {
      throw new Error('unknown Bitbucket account action');
    }
    addAnother = nextAction === 'add';
  }
  ...
  for (const { account, auth } of candidates) {
    ...
    if (!Array.isArray(repositories) || repositories.length === 0) {
      throw new Error(`Select at least one repository for ${account.credentialUsername}@bitbucket.org`);
    }
    accounts.push({ ...account, repositories });
  }
  return accounts;
...
export function reviewerBackendOptions(agents, { allowCustom = true } = {}) {
  ...
  if (allowCustom) agentOptions.push({ value: 'custom', label: 'Custom command...' });
  return agentOptions;
}
