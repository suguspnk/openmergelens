...
async function acquireWindowsRetentionLock(
  ...
  for (let attempt = 0; attempt < retryLimit; attempt += 1) {
    ...
    try {
      ...
      if (err.code !== 'EEXIST') {
        // O_EXCL established ownership, but there is no descriptor-relative
        // rename primitive in Node's Windows API. Keep the marker inode in
        // place on failure instead of checking then renaming a pathname that
        // another actor could replace between those operations.
      ...
      try {
        ...
        // A stale-looking marker is still untrusted data. Node has no
        // descriptor-relative rename on Windows, so even a verified stale
        // inode cannot safely be reclaimed through its pathname: a newer
        // owner could replace that name after this probe and be overwritten.
        // Keep waiting and fail closed below; never steal the lock.
      } catch (probeError) {
        if (
