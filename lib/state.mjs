...
const WINDOWS_RETENTION_LOCK_OWNER_MAX_AGE_MS = 30_000;
...
// Linux exposes a monotonic process start token in /proc/<pid>/stat.  It is
// the only portable-enough identity available to bind a marker's PID to the
// process that created it; a PID by itself can be reused after a crash.  On
// platforms without an equivalent runtime primitive we deliberately return
// null and only reclaim markers whose PID is demonstrably absent.
async function processStartIdentity(
  ...
  try {
    ...
    // The fields after comm start with state (field 3). starttime is field
    // 22, i.e. token 19 in this suffix. Keep it as an opaque string because
    // its unit and range are kernel-defined.
    const fields = stat.slice(closeParen + 1).trim().split(/\s+/u);
    return typeof fields[19] === 'string' && fields[19] ? fields[19] : null;
  } catch {
    return null;
  }
}
...
async function acquireWindowsRetentionLock(
  lockPath,
  {
    ...
    reclaimPath,
    now = Date.now,
    getProcessStartIdentity = (pid) => processStartIdentity(pid, { platform }),
  } = {},
) {
  ...
  for (let attempt = 0; attempt < WINDOWS_RETENTION_LOCK_RETRY_LIMIT; attempt += 1) {
    ...
    try {
      ...
      // Capture the inode while the O_EXCL claim is still held. Every later
      // failure path either relocates this exact inode or leaves it in place;
      // it must never clean up a pathname that may have been substituted.
      ...
      try {
        startIdentity = await getProcessStartIdentity(process.pid);
      } catch {
        // Missing process-start identity is an explicit fail-closed mode for
        // stale reclamation, not an acquisition failure.
      }
      const ownerPayload = Buffer.from(JSON.stringify({
        version: WINDOWS_RETENTION_LOCK_OWNER_VERSION,
        pid: process.pid,
        createdAt: now(),
        ...(typeof startIdentity === 'string' && startIdentity
          ? { startIdentity }
          : {}),
      }) + '\n', 'utf8');
      ...
      if (err.code !== 'EEXIST') {
        // O_EXCL has already established ownership. Close the descriptor and
        // relocate only after rebinding the pathname to the captured inode;
        // if that proof is unavailable, retaining the marker is safer than
        // unlinking or renaming an unrelated file.
        let cleanupError;
        if (ownerHandle) {
          try {
      ...
      try {
        ...
        const markerAge = Number(now()) - Number(opened.mtimeMs);
        const ageIsStale = Number.isFinite(markerAge) &&
          markerAge >= WINDOWS_RETENTION_LOCK_OWNER_MAX_AGE_MS;
        ...
        if (
          ...
          if (liveness === 'absent') {
            ...
            // A live PID is reclaimable only when its verified process-start
            // token proves that it is a different process. If the token
            // cannot be read, fail closed and keep waiting.
            const currentStartIdentity = await getProcessStartIdentity(owner.pid);
            ownerIsStale = Boolean(
              currentStartIdentity && currentStartIdentity !== owner.startIdentity,
            );
          }
        }
        // An empty marker can only be reclaimed after its filesystem liveness
        // window has elapsed. This covers a crash between O_EXCL creation and
        // the owner payload write without guessing about a live writer.
        staleOwner = ageIsStale && (ownerIsStale || bytesRead === 0);
      } catch (probeError) {
        if (
      ...
      if (staleOwner && reclaimPath) {
        try {
          // Rebind the still-open probe immediately before the pathname
          // rename. This closes the check/reclaim gap for replacement races;
          // a concurrent reclaimer can win only by making this identity check
          // fail or by making rename return EEXIST/ENOENT.
          ...
          if (
            ...
            throw new Error('review state retention lock identity changed before reclaim');
          }
          await reclaimRename(lockPath, reclaimPath);
          await probe?.close().catch(() => {});
          probe = undefined;
          ...
          // A replacement or concurrent reclaimer wins the race. Never
          // unlink the pathname; continue the bounded acquisition wait.
        }
      }
      await probe?.close().catch(() => {});
...
async function openWindowsTemporary(
  ...
  // Reclaimed crash markers are retained under the same bounded temporary
  // naming contract; never delete the inode through an unverified pathname.
  const reclaimPath = path.join(
    parentPath,
    `${path.basename(stateFile)}.tmp-${process.pid}-${randomUUID()}`,
  );
