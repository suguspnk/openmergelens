...
async function openWindowsTemporary(
  ...
  const lockPath = `${stateFile}.retention-lock`;
  return withWindowsRetentionQueue(parentPath, async () => {
    ...
    try {
      ...
      // If capacity is exhausted, move the marker aside rather than delete
      // it. This releases the lock without unsafe pathname cleanup while
      // preserving operator evidence of the rejected reservation.
      try {
        const evidencePath = `${lockPath}.blocked-${process.pid}-${randomUUID()}`;
        await reserveRename(lockPath, evidencePath);
      } catch {
        // A failed move leaves the marker in place, so later saves fail
        // closed on the bounded lock wait instead of racing the namespace.
      }
