...
function windowsIdentity(stats) {
  const dev = normalizeWindowsIdentityPart(stats.dev);
  const ino = normalizeWindowsIdentityPart(stats.ino);
...
export function sameFileIdentity(
  ...
  if (platform === 'win32') {
    ...
    const mixedRepresentation = typeof left.dev !== typeof right.dev;
    ...
    const requireDeviceMatch = allowMixedHandlePathVolume
      ? (typeof left.dev === typeof right.dev && typeof left.dev !== 'bigint')
      : true;
    if (
      leftIdentity.ino !== rightIdentity.ino ||
      (requireDeviceMatch && leftIdentity.dev !== rightIdentity.dev)
    ) {
      unsupported();
      return false;
    }
    // A mixed number/BigInt pair is the hosted handle/path representation
    // mismatch described above. Do not feed it back through the path-to-path
    // volume check: the two observations' dev fields are not comparable at
    // this boundary. Callers that compare two pathname observations invoke
    // samePathIdentity directly, which always retains the volume check.
    if (
      requireVolumeMatch &&
      !mixedRepresentation &&
      !samePathIdentity(left, right, { platform, label })
    ) {
      unsupported();
      return false;
    }
  ...
  for (let attempt = 0; attempt < retryLimit; attempt += 1) {
    ...
    try {
      ...
      try {
        ...
        const pathStats = await lstat(guardPath, BIGINT_STATS_OPTIONS);
        if (
          !pathStats.isFile() ||
          pathStats.isSymbolicLink() ||
  ...
  for (let attempt = 0; attempt < retryLimit; attempt += 1) {
    ...
    try {
      ...
      try {
        ...
        const pathStats = await lstat(lockPath, BIGINT_STATS_OPTIONS);
        if (
          !pathStats.isFile() ||
          pathStats.isSymbolicLink() ||
          !sameFileIdentity(ownerStats, pathStats, {
