...
function sameFileIdentity(left, right) {
  ...
  // A descriptor/path binding is only proven by the kernel identity tuple.
  // In particular, timestamps and mode bits are mutable metadata and cannot
  // establish that two paths name the same object.
  const hasValidDeviceIdentity = [left, right].every((stats) =>
    Number.isInteger(stats.dev) && stats.dev > 0 &&
    Number.isInteger(stats.ino) && stats.ino > 0,
  );
