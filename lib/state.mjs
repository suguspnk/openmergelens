...
export async function loadState(
  ...
  try {
    ...
    if (!sameFileIdentity(handleStats, pathStats, { platform })) {
...
async function assertNoWindowsReparseAncestors(parentPath, phase) {
  ...
  if (process.platform !== 'win32' || !path.win32.isAbsolute(parentPath)) return;
...
async function verifyPrivateParent(
  ...
  parentHandle,
  phase,
  { platform = process.platform, realpath = fsRealpath } = {},
) {
  ...
  if (platform === 'win32') {
    await assertNoWindowsReparseAncestors(parentPath, phase);
  }
  assertPrivateParentStats(handleStats, { platform });
  if (
    !pathStats.isDirectory() ||
    pathStats.isSymbolicLink() ||
    !sameFileIdentity(handleStats, pathStats, { platform }) ||
...
async function verifyParentIdentity(
  ...
  if (
    ...
    !sameFileIdentity(expectedIdentity.handleStats, currentStats, { platform })
  ) {
    throw new Error(`review state parent directory identity changed ${phase}`);
  }
}
...
async function verifyPathIdentity(
  ...
  if (
    ...
    !sameFileIdentity(expectedHandleStats, currentStats, { platform })
  ) {
...
export async function saveState(
  ...
  try {
    ...
    if (
      ...
      !sameFileIdentity(handleStats, committedStats, { platform }) ||
