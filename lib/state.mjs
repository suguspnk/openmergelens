...
export async function loadState(
  ...
  try {
    ...
    const pathStats = await lstat(stateFile);
...
function sameFileIdentity(left, right) {
  ...
  // Windows file handles and path-based lstat can expose different volume or
  // file-index values on hosted filesystems even when they refer to the same
  // directory. Keep the descriptor/path binding strict on POSIX, while using
  // the stable creation/change metadata available from both Windows stats as
  // the fallback identity tuple. A replacement directory receives a new
  // birth/change timestamp and therefore still fails closed at the race
  // checks below.
...
export async function saveState(
  ...
  try {
    ...
    try {
      const targetStats = await lstat(stateFile);
