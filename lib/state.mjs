...
export function sameFileIdentity(
  ...
  if (platform === 'win32') {
    ...
    // fallback only: validate safe volume/file values, then bind the handle
    // and path by file index. Path-to-path callers separately compare the
    // complete volume/file tuple with samePathIdentity.
    const hasValidFileIdentity = [left, right].every((stats) =>
      typeof stats.ino === 'bigint' && stats.ino > 0n,
    );
    if (hasValidFileIdentity) {
    ...
    if (
      ...
      left.ino !== right.ino ||
      (requireVolumeMatch && left.dev !== right.dev)
    ) {
      unsupported();
      return false;
    }
