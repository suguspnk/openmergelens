...
export function sameFileIdentity(
  ...
  if (platform === 'win32') {
    ...
    // Mixed numeric/BigInt observations are only safe when the normalized
    // volume identifiers agree. A representation mismatch cannot establish
    // that two equal volume-local file indexes belong to the same volume.
    // Fail closed on differing volumes before any read, chmod, or write.
    const requireDeviceMatch = true;
    if (
