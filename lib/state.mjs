...
export function sameFileIdentity(
  ...
  if (platform === 'win32') {
    ...
    // some Windows Node/filesystem combinations, so the non-volume-bound
    // identity uses the exact file index. Numeric (or mixed) observations
    // retain the complete volume/file tuple to avoid accepting an ino-only
    // cross-volume match. Callers that need pathname binding pass
    // requireVolumeMatch and always compare the complete tuple below.
    const requireDeviceMatch = requireVolumeMatch ||
