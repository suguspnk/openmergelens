...
export function sameFileIdentity(
  ...
  const unsupported = () => {
    ...
    throw new Error(
      `${label} identity is unsupported on this Windows filesystem: ` +
        'fs.Stats bigint dev/ino are unavailable or mismatched',
    );
  };
  ...
  // A descriptor/path binding is only proven by the exact kernel identity
  // tuple. Require bigint Stats values so large Windows file indexes and
  // device identifiers cannot be rounded by JavaScript numbers. Timestamps
  // and mode bits are mutable metadata and cannot establish object identity.
