...
export function samePathIdentity(
  ...
  const hasValidPathIdentity = [left, right].every((stats) =>
    typeof stats.dev === 'bigint' && stats.dev > 0n &&
    typeof stats.ino === 'bigint' && stats.ino > 0n,
  );
  if (!hasValidPathIdentity) {
    if (platform === 'win32') {
      throw new Error(
        `${label} identity is unsupported on this Windows filesystem: ` +
...
async function verifyPrivateParent(parentPath, parentHandle, phase) {
  ...
  return { handleStats, pathStats };
}

async function verifyParentIdentity(parentPath, expectedIdentity, phase) {
