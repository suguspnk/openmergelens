...
export async function loadState(
  ...
  let parentHandle;
  let handle;
  let missingStateAllowed = true;
  try {
    parentHandle = await open(parentPath, fsConstants.O_RDONLY | NO_FOLLOW);
    const parentStats = await verifyPrivateParent(
      parentPath,
      parentHandle,
      'before reading state',
    );
    const pathStats = await lstat(stateFile, BIGINT_STATS_OPTIONS);
    missingStateAllowed = false;
    ...
    assertUserOwnedFileStats(handleStats, 'review state file');
    if (!sameFileIdentity(handleStats, pathStats)) {
      throw new Error('review state file identity changed before reading');
    }
    await verifyParentIdentity(parentPath, parentStats, 'while opening state');
    if (hardenPermissions) {
      ...
      await hardenHandle(handle, PRIVATE_FILE_MODE);
    }
    let raw;
    ...
    await verifyParentIdentity(parentPath, parentStats, 'while reading state');
    ...
    if (missingStateAllowed && err.code === 'ENOENT') return {};
    throw err;
  } finally {
...
export function sameFileIdentity(
  left,
  right,
  { platform = process.platform, label = 'review state object' } = {},
) {
  const unsupported = () => {
  ...
  // Windows Node reports the volume/device identifier differently for path
  // and handle stats. The file-index identifier is the stable kernel object
  // identity across both calls, so compare only that exact bigint field on
  // Windows. Timestamps and mode bits are mutable metadata and cannot
  // establish object identity. On POSIX the exact (device, inode) tuple is
  // required.
  if (platform === 'win32') {
    const hasValidFileIdentity = [left, right].every((stats) =>
      typeof stats.ino === 'bigint' && stats.ino > 0n,
    );
    if (!hasValidFileIdentity || left.ino !== right.ino) {
      unsupported();
      return false;
    }
    return true;
...
function assertPrivateParentStats(stats) {
...
function assertUserOwnedFileStats(stats, label) {
  if (!stats.isFile()) {
...
async function verifyPrivateParent(parentPath, parentHandle, phase) {
  const [handleStats, pathStats] = await Promise.all([
    parentHandle.stat(BIGINT_STATS_OPTIONS),
    lstat(parentPath, BIGINT_STATS_OPTIONS),
  ]);
  assertPrivateParentStats(handleStats);
  if (!pathStats.isDirectory() || !sameFileIdentity(handleStats, pathStats)) {
    throw new Error(`review state parent directory identity changed ${phase}`);
  }
  return handleStats;
}

async function verifyParentIdentity(parentPath, expectedStats, phase) {
...
export async function saveState(
  ...
  try {
    parentHandle = await open(parentPath, fsConstants.O_RDONLY | NO_FOLLOW);
    parentStats = await verifyPrivateParent(
      parentPath,
      parentHandle,
      'before commit',
    );
    ...
    assertUserOwnedFileStats(handleStats, 'review state temporary file');
    if (
      !pathStats.isFile() ||
    ...
    await verifyParentIdentity(parentPath, parentStats, 'before commit');
    try {
    ...
    await verifyParentIdentity(parentPath, parentStats, 'before atomic commit');
    ...
    const [committedStats] = await Promise.all([
      ...
      verifyParentIdentity(parentPath, parentStats, 'after atomic commit'),
    ]);
    if (
    ...
    try {
      ...
      await flushParentHandle(parentHandle);
    } catch (err) {
      try {
