...
const MAX_BITBUCKET_POSTING_PLAN_ENTRIES = 100;
const MAX_BITBUCKET_POSTING_PLAN_KEY_CHARS = 1_024;
const MAX_BITBUCKET_COMMIT_ID_CHARS = 128;
export const BITBUCKET_POSTING_PLAN_UNREQUESTED_TTL_MS = 7 * 24 * 60 * 60 * 1_000;
export const BITBUCKET_TERMINAL_HEAD_RETENTION_MS = 30 * 24 * 60 * 60 * 1_000;
export const MAX_BITBUCKET_TERMINAL_HEAD_ENTRIES = 10_000;
const BITBUCKET_TERMINAL_DISPOSITION = 'posting-plan-expired-unrequested';
...
const WINDOWS_RETENTION_GUARD_NAME = '.openmergelens-retention.guard';
const WINDOWS_RETENTION_LOCK_BLOCKED = 'blocked\n';
const WINDOWS_RETENTION_LOCK_OWNER_VERSION = 1;
const WINDOWS_RETENTION_LOCK_OWNER_MAX_BYTES = 512;
const WINDOWS_RETENTION_LOCK_MAX_OWNER_AGE_MS = 30_000;
const WINDOWS_RETENTION_GUARD_OWNER_MAX_BYTES = 512;
const WINDOWS_RETENTION_CAPACITY_ERROR = 'ERETENTIONCAPACITY';
const WINDOWS_RETENTION_LOCK_UNSAFE_ERROR = 'ERETENTIONLOCKUNSAFE';
const WINDOWS_RETENTION_GUARD_CRASH_ERROR = 'ERETENTIONGUARDCRASH';
...
const REVIEW_ENTRY_FIELDS = new Set([
  ...
  'lastReviewedAt',
  'reviewMarkerVersion',
  'bitbucketRequestRemovedAt',
]);
const BITBUCKET_TERMINAL_HEAD_FIELDS = new Set([
  'bitbucketTerminalHeadSha',
  'terminalAt',
  'disposition',
]);
...
export function reviewerKey(account) {
...
export function parsePrKey(key) {
  ...
  let repo;
  try {
    repo = reviewerText?.startsWith('bitbucket.org@{')
      ? normalizeBitbucketRepository(repoText).toLowerCase()
      : normalizeRepository(repoText).toLowerCase();
  } catch {
    return null;
  }
  let reviewer = null;
  if (reviewerText !== null) {
    if (/^bitbucket\.org@\{[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\}$/iu.test(reviewerText)) {
      reviewer = reviewerText.toLowerCase();
    } else {
...
export function prKey(repo, number, reviewer) {
  ...
  const normalizedRepo = reviewer && reviewer.accountId !== undefined
    ? normalizeBitbucketRepository(repo)
    : normalizeRepository(repo);
  const pullRequest = `${normalizedRepo.toLowerCase()}#${number}`;
...
function validateState(
  ...
  for (const [key, entry] of Object.entries(state)) {
    ...
    // Terminal Bitbucket entries deliberately use a compact, distinct record
    // shape. Only Bitbucket state keys can contain one, so ordinary GitHub
    // entries do not pay an additional object-property lookup during large
    // legacy-state validation.
    if (
      parsedKey?.reviewer?.startsWith('bitbucket.org@{') &&
      isPlainObject(entry) &&
      Object.hasOwn(entry, 'bitbucketTerminalHeadSha')
    ) {
      if (!validBitbucketTerminalHead(entry)) {
        throw new Error(
          `Invalid review state entry "${key}" in ${stateFile}: ` +
            'expected a canonical Bitbucket terminal-head record',
        );
      }
      continue;
    }
    ...
    if (
      ...
      (
        entry.bitbucketRequestRemovedAt !== undefined &&
        (
          !parsedKey.reviewer.startsWith('bitbucket.org@{') ||
          !isCanonicalReviewTimestamp(entry.bitbucketRequestRemovedAt, nowMs)
        )
      ) ||
      (
...
function validateStateMetadata(metadata, stateFile) {
  ...
  const postingPlans = metadata.bitbucketPostingPlans;
  if (postingPlans !== undefined && !isPlainObject(postingPlans)) {
    throw new Error(
      `Invalid review state metadata in ${stateFile}: bitbucketPostingPlans must be an object`,
    );
  }
  const planEntries = Object.entries(postingPlans || {});
  if (planEntries.length > MAX_BITBUCKET_POSTING_PLAN_ENTRIES) {
    throw new Error(
      `Invalid review state metadata in ${stateFile}: too many Bitbucket posting plans`,
    );
  }
  for (const [key, plan] of planEntries) {
    if (!validBitbucketPostingPlan(key, plan)) {
      throw new Error(
        `Invalid review state metadata in ${stateFile}: Bitbucket posting plan is invalid`,
      );
    }
  }
  if (
...
function cloneStateMetadata(metadata) {
  ...
  if (isPlainObject(metadata.bitbucketPostingPlans)) {
    clone.bitbucketPostingPlans = Object.fromEntries(
      Object.entries(metadata.bitbucketPostingPlans).map(([key, plan]) => [
        key,
        {
          commitId: plan.commitId,
          body: plan.body,
          comments: plan.comments.map((finding) => ({ ...finding })),
          ...(plan.unrequestedAt ? { unrequestedAt: plan.unrequestedAt } : {}),
        },
      ]),
    );
  }
...
function validBitbucketPostingPlan(key, plan) {
  if (
    !key || key.length > MAX_BITBUCKET_POSTING_PLAN_KEY_CHARS ||
    !isPlainObject(plan) ||
    typeof plan.commitId !== 'string' || !plan.commitId ||
    plan.commitId.length > MAX_BITBUCKET_COMMIT_ID_CHARS ||
    /[\0\r\n]/u.test(plan.commitId) ||
    typeof plan.body !== 'string' || plan.body.length > MAX_REVIEW_SUMMARY_CHARS ||
    !Array.isArray(plan.comments) || plan.comments.length > MAX_REVIEW_FINDINGS ||
    (plan.unrequestedAt !== undefined && !isCanonicalIsoTimestamp(plan.unrequestedAt))
  ) return false;
  let textCharacters = plan.body.length;
  for (const finding of plan.comments) {
    if (
    ...
    textCharacters += finding.comment.length;
    if (textCharacters > MAX_REVIEW_TOTAL_TEXT_CHARS) return false;
  }
  return true;
}
...
function validBitbucketTerminalHead(entry) {
  if (
    !isPlainObject(entry) ||
    typeof entry.bitbucketTerminalHeadSha !== 'string' ||
    !entry.bitbucketTerminalHeadSha
  ) return false;
  const unknownField = Object.keys(entry)
    .find((field) => !BITBUCKET_TERMINAL_HEAD_FIELDS.has(field));
  return unknownField === undefined && Boolean(
    entry.bitbucketTerminalHeadSha &&
    entry.bitbucketTerminalHeadSha.length <= MAX_BITBUCKET_COMMIT_ID_CHARS &&
    !/[\0\r\n]/u.test(entry.bitbucketTerminalHeadSha) &&
    isCanonicalIsoTimestamp(entry.terminalAt) &&
    entry.disposition === BITBUCKET_TERMINAL_DISPOSITION,
  );
}

function withStateMetadata(current, additions = {}) {
  return {
    version: STATE_METADATA_VERSION,
    candidateCursors: isPlainObject(current?.candidateCursors)
      ? Object.fromEntries(Object.entries(current.candidateCursors))
      : {},
    ...(current?.reviewStateGcAfterKey !== undefined
      ? { reviewStateGcAfterKey: current.reviewStateGcAfterKey }
      : {}),
    ...(isPlainObject(current?.bitbucketPostingPlans)
      ? { bitbucketPostingPlans: current.bitbucketPostingPlans }
      : {}),
    ...additions,
  };
}
...
export async function loadState(
  ...
  try {
    ...
    try {
      ...
      pathProofStats = await lstat(stateFile, BIGINT_STATS_OPTIONS);
    } catch (err) {
    ...
    if (!samePathIdentity(pathStats, pathProofStats, {
      platform,
      canonicalVolume: parentIdentity.canonicalVolume,
    })) {
    ...
    if (!sameFileIdentity(handleStats, pathStats, {
      ...
      allowMixedHandlePathVolume: platform === 'win32',
    })) {
      throw new Error('review state file identity changed before reading');
    }
    if (!samePathIdentity(pathStats, pathStats, {
      platform,
      canonicalVolume: parentIdentity.canonicalVolume,
    })) {
      throw new Error('review state file identity changed before reading');
...
// Windows Node releases/filesystems have returned the same stat fields as
// either numbers or bigint values (and, for handle/path observations, the
// two forms can be mixed). Normalize only safe integer numbers so that the
// identity comparison remains exact. Never coerce an imprecise number or use
// mutable metadata such as timestamps as an identity substitute.
function normalizeWindowsIdentityPart(value) {
...
function windowsIdentityPart(value) {
  if (value === undefined || value === null || value === 0 || value === 0n) {
    return { value: null, available: false };
  }
  const normalized = normalizeWindowsIdentityPart(value);
  return normalized === null
    ? { value: null, available: false, invalid: true }
    : { value: normalized, available: true };
}

function windowsIdentity(stats) {
  return {
    dev: windowsIdentityPart(stats.dev),
    ino: windowsIdentityPart(stats.ino),
  };
}
...
export function sameFileIdentity(
  ...
  const unsupported = () => {
    ...
    throw new Error(
      `${label} identity is unsupported on this Windows filesystem: ` +
        'fs.Stats volume/file indexes are unavailable or mismatched',
    );
  };
  ...
  if (platform === 'win32') {
    ...
    // samePathIdentity.
    const identities = [windowsIdentity(left), windowsIdentity(right)];
    if (identities.some(({ dev, ino }) =>
      dev.invalid || ino.invalid || !ino.available ||
      (!allowMixedHandlePathVolume && !dev.available))) {
      unsupported();
      return false;
    }
    const [leftIdentity, rightIdentity] = identities;
    // BigInt handle/path stats are known to expose different volume values on
    // some Windows Node/filesystem combinations. When both observations use
    // the same representation, a caller that explicitly requires pathname
    // binding can compare the volume as well. A mixed number/BigInt pair is a
    // hosted-runtime representation mismatch: its exact file index remains
    // comparable, while comparing the differently represented volume fields
    // would reject valid handle/path observations on Windows. Path-to-path
    // callers must use samePathIdentity, which always compares the complete
    // normalized volume/file tuple independently of this handle binding.
    const requireDeviceMatch = !allowMixedHandlePathVolume &&
      leftIdentity.dev.available &&
      rightIdentity.dev.available;
    if (
      leftIdentity.ino.value !== rightIdentity.ino.value ||
      (requireDeviceMatch && leftIdentity.dev.value !== rightIdentity.dev.value)
    ) {
      unsupported();
      return false;
    }
...
// they refer to the same object. Path-to-path checks require the complete
// volume/file tuple when `dev` is available. On the constrained canonical
// Windows state path, callers may provide the already-realpathed volume root
// as an independent fallback when both path observations report an unavailable
// `dev`; mixed availability remains unsupported. `canonicalVolume` applies to
// both observations, while callers comparing observations captured at
// different times can provide `leftCanonicalVolume` and
// `rightCanonicalVolume` separately. Handle/path callers may explicitly opt
// into ino-only binding for the known representation divergence.
export function samePathIdentity(
  left,
  right,
  {
    platform = process.platform,
    label = 'review state path',
    canonicalVolume,
    leftCanonicalVolume = canonicalVolume,
    rightCanonicalVolume = canonicalVolume,
  } = {},
) {
  ...
  if (identities.some(({ dev, ino }) =>
    ...
    if (platform === 'win32') {
      throw new Error(
        `${label} identity is unsupported on this Windows filesystem: ` +
          'fs.Stats file indexes are unavailable',
      );
    }
    return false;
  }
  const [leftIdentity, rightIdentity] = identities;
  if (!leftIdentity.dev.available || !rightIdentity.dev.available) {
    if (platform === 'win32') {
      // A canonical volume root can replace the unavailable `dev` field only
      // when both independent pathname observations have the same missing
      // representation. A mixed available/unavailable pair is ambiguous and
      // must remain fail-closed; otherwise a test or an attacker could turn a
      // zero-volume observation plus a different volume into an apparent
      // match by relying on the root of only one pathname.
      if (!leftIdentity.dev.available &&
          !rightIdentity.dev.available &&
          normalizeCanonicalWindowsVolume(leftCanonicalVolume) !== null &&
          normalizeCanonicalWindowsVolume(rightCanonicalVolume) !== null) {
        if (normalizeCanonicalWindowsVolume(leftCanonicalVolume) !==
            normalizeCanonicalWindowsVolume(rightCanonicalVolume)) {
          return false;
        }
        return leftIdentity.ino.value === rightIdentity.ino.value;
      }
      throw new Error(
        `${label} identity is unsupported on this Windows filesystem: ` +
          'fs.Stats volume/file identities are unavailable',
      );
    }
    return false;
  }
  return leftIdentity.dev.value === rightIdentity.dev.value &&
    leftIdentity.ino.value === rightIdentity.ino.value;
}
...
function canonicalWindowsVolume(filePath) {
  if (typeof filePath !== 'string' || !filePath) return null;
  const resolvedPath = path.win32.normalize(path.win32.resolve(filePath));
  const root = path.win32.parse(resolvedPath).root;
  return root ? root.toLowerCase() : null;
}

function normalizeCanonicalWindowsVolume(volume) {
  if (typeof volume !== 'string' || !volume) return null;
  const normalized = path.win32.normalize(volume).toLowerCase();
  return path.win32.parse(normalized).root === normalized ? normalized : null;
}
...
async function verifyPrivateParent(
  ...
  const [handleStats, pathStats, pathProofStats] = await Promise.all([
  ...
  const canonicalVolume = canonicalWindowsVolume(canonicalPath);
  if (
    ...
    !samePathIdentity(pathStats, pathProofStats, {
      platform,
      canonicalVolume,
    })
  ) {
  ...
  return { handleStats, pathStats, canonicalPath, canonicalVolume };
}
...
async function verifyParentIdentity(
  ...
  if (
    ...
    !samePathIdentity(expectedIdentity.pathStats, currentStats, {
      platform,
      leftCanonicalVolume: expectedIdentity.canonicalVolume,
      rightCanonicalVolume: canonicalVolume,
    }) ||
...
async function verifyPathIdentity(
  ...
  const [currentStats, currentPathProof] = await Promise.all([
    lstat(targetPath, BIGINT_STATS_OPTIONS),
    lstat(targetPath, BIGINT_STATS_OPTIONS),
  ]);
  if (
    !currentStats.isFile() ||
    currentStats.isSymbolicLink() ||
    !samePathIdentity(currentStats, currentPathProof, {
      platform,
      canonicalVolume,
    }) ||
    !samePathIdentity(expectedPathStats, currentStats, {
      platform,
      canonicalVolume,
    }) ||
...
function retentionPidLiveness(pid) {
...
function retentionGuardCrashError(lockPath, detail = '') {
  const error = new Error(
    `review state temporary retention guard is abandoned or unsafe at ` +
    `${lockPath}; confirm no OpenMergeLens save is running, then remove ` +
    `the guard file${detail ? ` (${detail})` : ''}`,
  );
  error.code = WINDOWS_RETENTION_GUARD_CRASH_ERROR;
  return error;
}

async function acquireWindowsRetentionGuard(
  guardPath,
  {
    openFile = open,
    platform = process.platform,
    lstat = fsLstat,
    reclaimRename = rename,
    reclaimPath,
    now = Date.now,
    retryLimit = WINDOWS_RETENTION_LOCK_RETRY_LIMIT,
    retryDelayMs = WINDOWS_RETENTION_LOCK_RETRY_MS,
    canonicalVolume,
  } = {},
) {
  if (platform !== 'win32') return null;
  let sawInitializingMarker = false;
  for (let attempt = 0; attempt < retryLimit; attempt += 1) {
    let ownerHandle;
    let ownerStats;
    try {
      ownerHandle = await openFile(
        guardPath,
        fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | NO_FOLLOW,
        PRIVATE_FILE_MODE,
      );
      ownerStats = await ownerHandle.stat(BIGINT_STATS_OPTIONS);
      const ownerPayload = Buffer.from(JSON.stringify({
        version: WINDOWS_RETENTION_LOCK_OWNER_VERSION,
        pid: process.pid,
        createdAt: now(),
      }) + '\n', 'utf8');
      if (ownerPayload.byteLength > WINDOWS_RETENTION_GUARD_OWNER_MAX_BYTES) {
        throw retentionGuardCrashError(guardPath, 'owner marker is oversized');
      }
      let payloadHandle;
      try {
        payloadHandle = await openFile(guardPath, fsConstants.O_WRONLY | NO_FOLLOW);
        const payloadStats = await payloadHandle.stat(BIGINT_STATS_OPTIONS);
        const [pathStats, pathProofStats] = await Promise.all([
          lstat(guardPath, BIGINT_STATS_OPTIONS),
          lstat(guardPath, BIGINT_STATS_OPTIONS),
        ]);
        if (
          !pathStats.isFile() ||
          pathStats.isSymbolicLink() ||
          !sameFileIdentity(ownerStats, payloadStats, {
            platform,
            requireVolumeMatch: true,
            allowMixedHandlePathVolume: true,
          }) ||
          !sameFileIdentity(ownerStats, pathStats, {
            platform,
            requireVolumeMatch: true,
            allowMixedHandlePathVolume: true,
          }) ||
          ...
          throw retentionGuardCrashError(
            guardPath,
            'owner marker identity changed while claiming ownership',
          );
        }
        const writeResult = await payloadHandle.write(
          ownerPayload,
          0,
          ownerPayload.byteLength,
          0,
        );
        if (
          !writeResult ||
          writeResult.bytesWritten !== ownerPayload.byteLength
        ) {
          throw new Error('review state temporary retention guard owner marker write was short');
        }
        await payloadHandle.datasync();
      } finally {
        await payloadHandle?.close().catch(() => {});
      }
      return {
        async release() {
          try {
            const heldStats = await ownerHandle.stat(BIGINT_STATS_OPTIONS);
            const [pathStats, pathProofStats] = await Promise.all([
              lstat(guardPath, BIGINT_STATS_OPTIONS),
              lstat(guardPath, BIGINT_STATS_OPTIONS),
            ]);
            if (
              !pathStats.isFile() ||
              pathStats.isSymbolicLink() ||
              !sameFileIdentity(heldStats, pathStats, {
                platform,
                requireVolumeMatch: true,
                allowMixedHandlePathVolume: true,
              }) ||
              ...
              throw retentionGuardCrashError(
                guardPath,
                'owner marker identity changed before release',
              );
            }
            // The guard path remains owned until this final identity check.
            // Contenders cannot replace it while it exists; a replacement by
            // an out-of-band actor fails closed above rather than deleting a
            // different file. Windows' sharing rules also prevent mutation of
            // the open marker by ordinary callers while the owner is active.
      ...
      if (error?.code !== 'EEXIST') {
        // O_EXCL established ownership of this exact inode. If initialization
        // fails before the owner payload is durable, relocate that inode under
        // a generated temporary name while the descriptor remains held. A
        // pathname unlink (or an unverified rename) could remove a competing
        // marker after a replacement race, so bind the rename to the held
        // descriptor identity first and retain the inode if that proof fails.
        if (ownerHandle && reclaimPath) {
          try {
            const heldStats = ownerStats || await ownerHandle.stat(BIGINT_STATS_OPTIONS);
            const [pathStats, pathProofStats] = await Promise.all([
              lstat(guardPath, BIGINT_STATS_OPTIONS),
              lstat(guardPath, BIGINT_STATS_OPTIONS),
            ]);
            if (
              ...
              throw retentionGuardCrashError(
                guardPath,
                'owner marker identity changed while releasing initialization failure',
              );
            }
            await reclaimRename(guardPath, reclaimPath);
          } catch (cleanupError) {
      ...
      // Guard recovery deliberately has no pathname reclaim operation. A
      // stale-guard check followed by rename/unlink would itself race with a
      // released owner and could move a newer guard. A verified dead owner is
      // therefore surfaced as an operator-recovery condition; only the guard
      // crash may fail closed. All contenders take this guard before reading
      // or creating the main retention lock.
      let probe;
      try {
        probe = await openFile(guardPath, fsConstants.O_RDONLY | NO_FOLLOW);
        const opened = await probe.stat(BIGINT_STATS_OPTIONS);
        if (
          !opened.isFile() ||
          opened.isSymbolicLink() ||
          opened.size > BigInt(WINDOWS_RETENTION_GUARD_OWNER_MAX_BYTES)
        ) {
          throw retentionGuardCrashError(guardPath, 'marker is not a bounded regular file');
        }
        const contents = Buffer.alloc(WINDOWS_RETENTION_GUARD_OWNER_MAX_BYTES + 1);
        let bytesRead = 0;
        while (bytesRead < contents.length) {
          const read = await probe.read(contents, bytesRead, contents.length - bytesRead, bytesRead);
          bytesRead += read.bytesRead;
          if (read.bytesRead === 0) break;
        }
        if (bytesRead > WINDOWS_RETENTION_GUARD_OWNER_MAX_BYTES) {
          throw retentionGuardCrashError(guardPath, 'marker is oversized');
        }
        let owner;
        let initializing = bytesRead === 0 || !contents.subarray(0, bytesRead).includes(0x0a);
        try {
          owner = JSON.parse(contents.toString('utf8', 0, bytesRead));
        } catch {
          if (!initializing) {
            throw retentionGuardCrashError(guardPath, 'owner marker is incomplete or malformed');
          }
        }
        if (initializing) {
          sawInitializingMarker = true;
        ...
        if (
          owner?.version !== WINDOWS_RETENTION_LOCK_OWNER_VERSION ||
          !Number.isInteger(owner.pid) ||
          owner.pid <= 0 ||
          !Number.isFinite(owner.createdAt)
        ) {
          throw retentionGuardCrashError(guardPath, 'owner marker is not verifiable');
        }
        const liveness = retentionPidLiveness(owner.pid);
        if (liveness === 'absent') {
          throw retentionGuardCrashError(guardPath, 'owner process is no longer live');
        }
        if (liveness === 'unknown') {
          throw retentionGuardCrashError(guardPath, 'owner process liveness is unknown');
        }
        // An initialization marker may become a complete, live owner marker
        // on a later probe. Once that proof succeeds, exhaustion is an
        // ordinary contention outcome rather than evidence of a crashed
        // initialization that requires operator removal.
        sawInitializingMarker = false;
      } catch (probeError) {
        // The guard owner may release its marker after O_EXCL reported it but
        // before this contender opens it for inspection. There is no marker
        // to trust or reclaim in that case; restart acquisition so a new
        // owner can be claimed normally.
        if (probeError?.code === 'ENOENT') continue;
        if (probeError?.code === WINDOWS_RETENTION_GUARD_CRASH_ERROR) throw probeError;
        throw retentionGuardCrashError(guardPath, 'marker could not be safely inspected');
      } finally {
        await probe?.close().catch(() => {});
      }
      await new Promise((resolve) => setTimeout(resolve, retryDelayMs));
  ...
  if (sawInitializingMarker) {
    throw retentionGuardCrashError(
      guardPath,
      'owner marker initialization did not complete within the retry limit',
    );
  }
  throw new Error('review state temporary retention guard is unavailable');
}
...
async function acquireWindowsRetentionLock(
  ...
  for (let attempt = 0; attempt < retryLimit; attempt += 1) {
    ...
    try {
      ...
      // Capture the inode while the O_EXCL claim is still held. If a later
      // write fails, retain this marker rather than cleaning up a pathname
      // that may have been substituted.
      await ownerHandle.stat(BIGINT_STATS_OPTIONS);
      // Keep the descriptor offset at zero so the same handle becomes the
      // replacement temporary file after the pathname rename. A bounded,
      // descriptor-relative owner payload lets a later process distinguish a
      // live lock from one abandoned by a crashed owner.
      const ownerPayload = Buffer.from(JSON.stringify({
        version: WINDOWS_RETENTION_LOCK_OWNER_VERSION,
        pid: process.pid,
        createdAt: now(),
      }) + '\n', 'utf8');
      if (ownerPayload.byteLength > WINDOWS_RETENTION_LOCK_OWNER_MAX_BYTES) {
        throw new Error('review state temporary retention lock owner marker is oversized');
      }
      // Write through a second descriptor so the lock handle's offset remains
      // zero for the eventual temporary-state payload. Rebind both handles
      // before writing; if the pathname was substituted, fail closed.
      let payloadHandle;
      try {
        payloadHandle = await openFile(
          lockPath,
          fsConstants.O_WRONLY | NO_FOLLOW,
        );
        const [ownerStats, payloadStats] = await Promise.all([
          ownerHandle.stat(BIGINT_STATS_OPTIONS),
          payloadHandle.stat(BIGINT_STATS_OPTIONS),
        ]);
        if (!sameFileIdentity(ownerStats, payloadStats, {
          platform,
          requireVolumeMatch: platform === 'win32',
          allowMixedHandlePathVolume: true,
        })) {
          throw new Error('review state temporary retention lock identity changed while claiming ownership');
        }
        const [pathStats, pathProofStats] = await Promise.all([
          lstat(lockPath, BIGINT_STATS_OPTIONS),
          lstat(lockPath, BIGINT_STATS_OPTIONS),
        ]);
        if (
          !pathStats.isFile() ||
        ...
        await payloadHandle.writeFile(ownerPayload);
        await payloadHandle.datasync();
      } finally {
        if (payloadHandle) await payloadHandle.close().catch(() => {});
      }
      return ownerHandle;
    } catch (err) {
      if (err.code !== 'EEXIST') {
        // The parent-scoped guard excludes every OpenMergeLens contender from
        // this pathname while the marker is being rebound. Retain a failed
        // claim as a generated temporary artifact so a payload-write crash
        // cannot strand the main lock name permanently.
        if (ownerHandle && reclaimPath) {
          try {
            const heldStats = await ownerHandle.stat(BIGINT_STATS_OPTIONS);
            const [pathStats, pathProofStats] = await Promise.all([
            ...
            if (
              ...
              throw new Error('review state temporary retention lock identity changed while releasing failed claim');
            }
            await reclaimRename(lockPath, reclaimPath);
          } catch (cleanupError) {
      ...
      try {
        ...
        if (opened.size > BigInt(WINDOWS_RETENTION_LOCK_OWNER_MAX_BYTES)) {
          const unsafe = new Error(
        ...
        const contents = Buffer.alloc(WINDOWS_RETENTION_LOCK_OWNER_MAX_BYTES + 1);
        let bytesRead = 0;
        while (bytesRead < contents.length) {
        ...
        if (bytesRead > WINDOWS_RETENTION_LOCK_OWNER_MAX_BYTES) {
          const unsafe = new Error(
            'review state temporary retention lock is oversized',
          );
          unsafe.code = WINDOWS_RETENTION_LOCK_UNSAFE_ERROR;
        ...
        const payload = contents.toString('utf8', 0, bytesRead);
        let owner;
        try {
        ...
        const ownerIsStale = owner?.version === WINDOWS_RETENTION_LOCK_OWNER_VERSION &&
          Number.isInteger(owner.pid) &&
          owner.pid > 0 &&
          Number.isFinite(owner.createdAt) &&
          retentionPidLiveness(owner.pid) === 'absent';
        const ownerIsLive = owner?.version === WINDOWS_RETENTION_LOCK_OWNER_VERSION &&
          Number.isInteger(owner.pid) &&
          owner.pid > 0 &&
          Number.isFinite(owner.createdAt) &&
          retentionPidLiveness(owner.pid) === 'live';
        if (
          ownerIsLive &&
          now() - owner.createdAt > WINDOWS_RETENTION_LOCK_MAX_OWNER_AGE_MS
        ) {
          throw retentionGuardCrashError(
            lockPath,
            'owner marker is old but its PID is still live; confirm no save is running before removal',
          );
        }
        // The parent guard is held by this caller, so another cooperative
        // OpenMergeLens owner cannot be initializing this marker. An empty,
        // malformed, or unknown marker is therefore an abandoned claim and
        // may be reclaimed only after binding the opened inode to its path.
        if ((!ownerIsLive || ownerIsStale) && reclaimPath) {
          const rebound = await probe.stat(BIGINT_STATS_OPTIONS);
          const [pathStats, pathProofStats] = await Promise.all([
            lstat(lockPath, BIGINT_STATS_OPTIONS),
            lstat(lockPath, BIGINT_STATS_OPTIONS),
          ]);
          if (
            !pathStats.isFile() ||
            pathStats.isSymbolicLink() ||
            !sameFileIdentity(rebound, pathStats, {
              platform,
              requireVolumeMatch: platform === 'win32',
              allowMixedHandlePathVolume: true,
            }) ||
            ...
            throw new Error(
              'review state temporary retention lock identity changed before reclaim',
            );
          }
          // The guard makes this check/reclaim pair exclusive. A competing
          // contender cannot remove and recreate lockPath between the final
          // identity check and rename, closing the stale-reclaim race.
          await reclaimRename(lockPath, reclaimPath);
          await probe.close().catch(() => {});
          probe = undefined;
          continue;
        ...
        if (probeError?.code === WINDOWS_RETENTION_GUARD_CRASH_ERROR) {
          throw probeError;
        }
        if (
...
async function openWindowsTemporary(
  ...
  temporaryPath,
  {
    ...
    parentIdentity,
    realpath = fsRealpath,
    retentionLockRetryLimit = WINDOWS_RETENTION_LOCK_RETRY_LIMIT,
    retentionLockRetryDelayMs = WINDOWS_RETENTION_LOCK_RETRY_MS,
  } = {},
) {
  if (platform !== 'win32') {
    return openFile(
  ...
  const guardPath = path.join(parentPath, WINDOWS_RETENTION_GUARD_NAME);
  // Reclaimed marker inodes are retained under the same bounded temporary
  // naming contract. The guard itself is never auto-reclaimed: a stale guard
  // is an operator-recovery condition because reclaiming its pathname would
  // recreate the check/rename race this guard exists to close.
  const reclaimPath = `${stateFile}.tmp-${process.pid}-${randomUUID()}`;
  const guardReclaimPath = `${stateFile}.tmp-${process.pid}-${randomUUID()}`;
  return withWindowsRetentionQueue(parentPath, async () => {
    const guard = await acquireWindowsRetentionGuard(guardPath, {
      openFile,
      platform,
      lstat,
      reclaimRename: reserveRename,
      reclaimPath: guardReclaimPath,
      retryLimit: retentionLockRetryLimit,
      retryDelayMs: retentionLockRetryDelayMs,
      canonicalVolume: parentIdentity.canonicalVolume,
    });
    let lockHandle;
    try {
      lockHandle = await acquireWindowsRetentionLock(lockPath, {
        openFile,
        platform,
        lstat,
        reclaimRename: reserveRename,
      ...
      // The Windows retention lock carries a bounded owner marker while it
      // is contended. Once its inode becomes the state replacement, remove
      // that marker before the caller writes the JSON payload; otherwise a
      // shorter state document leaves marker bytes after the JSON and the
      // next load fails with a trailing-data parse error.
      ...
      await guard.release().catch((guardError) => {
        error.guardError = guardError;
      });
      throw error;
    }
  });
...
export async function saveState(
  ...
  try {
    ...
    handle = await openWindowsTemporary(parentPath, temporaryPath, {
      ...
      parentIdentity,
      realpath,
      retentionLockRetryLimit,
      retentionLockRetryDelayMs,
    });
    ...
    if (
      ...
      !samePathIdentity(pathStats, pathStats, {
        platform,
        canonicalVolume: parentIdentity.canonicalVolume,
      })
    ) {
    ...
    if (
      ...
      !samePathIdentity(temporaryPathStats, pathStats, {
        platform,
        canonicalVolume: parentIdentity.canonicalVolume,
      })
    ) {
    ...
    if (
      ...
      !samePathIdentity(temporaryPathStats, committedStats, {
        platform,
        canonicalVolume: parentIdentity.canonicalVolume,
      })
    ) {
...
export function reconcileBitbucketReviewRequests(
  state,
  { account, repo, requestedNumbers, observedAt },
) {
  if (
    !isPlainObject(state) ||
    typeof repo !== 'string' ||
    !repo ||
    !(requestedNumbers instanceof Set) ||
    [...requestedNumbers].some((number) => !Number.isSafeInteger(number) || number <= 0) ||
    !isCanonicalIsoTimestamp(observedAt)
  ) {
    throw new Error('Bitbucket review-request reconciliation input is invalid');
  }
  const reviewer = accountKey(account);
  const normalizedRepo = normalizeBitbucketRepository(repo).toLowerCase();
  let changed = 0;
  for (const [key, entry] of Object.entries(state)) {
    const parsed = parsePrKey(key);
    if (
      parsed?.reviewer !== reviewer ||
      parsed.repo !== normalizedRepo ||
      !isPlainObject(entry) ||
      typeof entry.lastReviewedSha !== 'string' ||
      requestedNumbers.has(parsed.number) ||
      entry.bitbucketRequestRemovedAt !== undefined
    ) continue;
    state[key] = { ...entry, bitbucketRequestRemovedAt: observedAt };
    changed += 1;
  }
  return { changed: changed > 0, marked: changed };
}

export function clearBitbucketReviewRequestRemoval(state, key) {
  const normalizedKey = normalizePrKey(key);
  const entry = state?.[normalizedKey];
  if (!isPlainObject(entry) || entry.bitbucketRequestRemovedAt === undefined) return false;
  const next = { ...entry };
  delete next.bitbucketRequestRemovedAt;
...
export function expireReviewState(state, nowMs = Date.now()) {
  ...
  for (const [key, entry] of Object.entries(state)) {
    if (key === STATE_METADATA_KEY) continue;
    if (key.startsWith('bitbucket.org@{') && validBitbucketTerminalHead(entry)) continue;
...
export function recordCandidateCursor(state, cursorKey, offset) {
  ...
  state[STATE_METADATA_KEY] = withStateMetadata(current, {
...
function orderedReviewStateEntry(entry, gcPosition) {
  ...
  if (entry.bitbucketRequestRemovedAt !== undefined) {
    ordered.bitbucketRequestRemovedAt = entry.bitbucketRequestRemovedAt;
  }
  return ordered;
}
...
export function bitbucketPostingPlanFor(state, key, commitId) {
  const plan = state?.[STATE_METADATA_KEY]?.bitbucketPostingPlans?.[normalizePrKey(key)];
  if (!plan || plan.commitId !== commitId) return null;
  return {
    commitId: plan.commitId,
    body: plan.body,
    comments: plan.comments.map((finding) => ({ ...finding })),
...
export function hasBitbucketPostingPlan(state, key) {
  const plans = state?.[STATE_METADATA_KEY]?.bitbucketPostingPlans;
  return isPlainObject(plans) && Object.prototype.hasOwnProperty.call(
    plans,
    normalizePrKey(key),
  );
}

export function bitbucketTerminalHeadFor(state, key, commitId) {
  const terminal = state?.[normalizePrKey(key)];
  if (!validBitbucketTerminalHead(terminal) || terminal.bitbucketTerminalHeadSha !== commitId) {
    return null;
  }
  return { ...terminal };
}

export function pruneBitbucketTerminalHeads(state, observedAt) {
  if (!isPlainObject(state) || !isCanonicalIsoTimestamp(observedAt)) {
    throw new Error('Bitbucket terminal-head pruning input is invalid');
  }
  const observedMilliseconds = Date.parse(observedAt);
  const retained = [];
  let pruned = 0;
  for (const [key, entry] of Object.entries(state)) {
    if (!validBitbucketTerminalHead(entry)) continue;
    if (observedMilliseconds - Date.parse(entry.terminalAt) >= BITBUCKET_TERMINAL_HEAD_RETENTION_MS) {
      delete state[key];
      pruned += 1;
    } else {
  ...
  retained.sort(([leftKey, left], [rightKey, right]) =>
    Date.parse(left.terminalAt) - Date.parse(right.terminalAt) ||
    (leftKey < rightKey ? -1 : leftKey > rightKey ? 1 : 0));
  for (const [key] of retained.slice(0, Math.max(0, retained.length - MAX_BITBUCKET_TERMINAL_HEAD_ENTRIES))) {
    delete state[key];
    pruned += 1;
  }
  return { changed: pruned > 0, pruned };
}

export function recordBitbucketPostingPlan(state, key, plan) {
  const normalizedKey = normalizePrKey(key);
  if (!validBitbucketPostingPlan(normalizedKey, plan)) {
    throw new Error('Bitbucket posting plan is invalid');
  }
  const current = state[STATE_METADATA_KEY];
  const plans = isPlainObject(current?.bitbucketPostingPlans)
    ? { ...current.bitbucketPostingPlans }
    : {};
  const existing = plans[normalizedKey];
  if (existing?.commitId === plan.commitId) {
    return bitbucketPostingPlanFor(state, normalizedKey, plan.commitId);
  }
  if (!existing && Object.keys(plans).length >= MAX_BITBUCKET_POSTING_PLAN_ENTRIES) {
    throw new Error('Bitbucket posting plan limit reached');
  }
  plans[normalizedKey] = {
    commitId: plan.commitId,
    body: plan.body,
  ...
  state[STATE_METADATA_KEY] = withStateMetadata(current, {
    bitbucketPostingPlans: plans,
  });
  return bitbucketPostingPlanFor(state, normalizedKey, plan.commitId);
}

export function clearBitbucketPostingPlan(state, key) {
  const current = state[STATE_METADATA_KEY];
  if (!isPlainObject(current?.bitbucketPostingPlans)) return false;
  const normalizedKey = normalizePrKey(key);
  if (!Object.prototype.hasOwnProperty.call(current.bitbucketPostingPlans, normalizedKey)) return false;
  const plans = { ...current.bitbucketPostingPlans };
  delete plans[normalizedKey];
  const nextMetadata = withStateMetadata(current, {
    ...(Object.keys(plans).length > 0 ? { bitbucketPostingPlans: plans } : {}),
  });
  if (Object.keys(plans).length === 0) delete nextMetadata.bitbucketPostingPlans;
  state[STATE_METADATA_KEY] = nextMetadata;
  return true;
}

export function reconcileBitbucketPostingPlans(
  state,
  { account, repo, requestedNumbers, observedAt },
) {
  if (
    typeof repo !== 'string' || !repo || !(requestedNumbers instanceof Set) ||
    [...requestedNumbers].some((number) => !Number.isSafeInteger(number) || number <= 0) ||
    !isCanonicalIsoTimestamp(observedAt)
  ) throw new Error('Bitbucket posting plan reconciliation input is invalid');
  const pruning = pruneBitbucketTerminalHeads(state, observedAt);
  const current = state[STATE_METADATA_KEY];
  if (!isPlainObject(current?.bitbucketPostingPlans)) {
    return { changed: pruning.changed, expired: 0, marked: 0, resumed: 0, deferred: 0, pruned: pruning.pruned };
  }
  const reviewer = accountKey(account);
  const normalizedRepo = normalizeBitbucketRepository(repo).toLowerCase();
  const observedMilliseconds = Date.parse(observedAt);
  ...
  let resumed = 0;
  let deferred = 0;
  let terminalCount = Object.values(state).filter(validBitbucketTerminalHead).length;
  for (const [key, plan] of Object.entries(plans)) {
    const parsed = parsePrKey(key);
    if (!parsed || parsed.reviewer !== reviewer || parsed.repo !== normalizedRepo) continue;
    if (requestedNumbers.has(parsed.number)) {
      if (plan.unrequestedAt !== undefined) {
        const { unrequestedAt: _ignored, ...active } = plan;
        plans[key] = active;
        resumed += 1;
      }
      continue;
    ...
    if (plan.unrequestedAt === undefined) {
      plans[key] = { ...plan, unrequestedAt: observedAt };
      marked += 1;
      continue;
    }
    if (observedMilliseconds - Date.parse(plan.unrequestedAt) < BITBUCKET_POSTING_PLAN_UNREQUESTED_TTL_MS) continue;
    const replacesTerminal = validBitbucketTerminalHead(state[key]);
    if (!replacesTerminal && terminalCount >= MAX_BITBUCKET_TERMINAL_HEAD_ENTRIES) {
      deferred += 1;
      continue;
    }
    state[key] = {
      bitbucketTerminalHeadSha: plan.commitId,
      terminalAt: observedAt,
      disposition: BITBUCKET_TERMINAL_DISPOSITION,
    };
    if (!replacesTerminal) terminalCount += 1;
    delete plans[key];
    expired += 1;
  }
  const changed = pruning.changed || expired + marked + resumed > 0;
  if (changed) {
    const nextMetadata = withStateMetadata(current, {
      ...(Object.keys(plans).length > 0 ? { bitbucketPostingPlans: plans } : {}),
    });
  ...
  return { changed, expired, marked, resumed, deferred, pruned: pruning.pruned };
}
