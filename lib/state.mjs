...
function sameFileIdentity(left, right) {
  if (process.platform === 'win32') {
    // Windows `ino` is the file-index identity returned by both handle.stat()
    // and path lstat(). Volume identifiers are not stable across the two
    // Node APIs on hosted runners, so require the genuine file index only;
    // never substitute timestamps or mode bits for identity.
