...
function migrateReviewStateProofQueue(state, metadata) {
  ...
  for (const scope of orderedScopes) {
    ...
    orderedKeys.push(...rotateAfter(keys, proofKeys[scope]));
  }
...
async function flushDirectoryHandle(handle) {
  try {
    await handle.sync();
  } catch (err) {
    if (
...
export async function saveState(
  ...
  state,
  {
    ...
    flushParentHandle = flushDirectoryHandle,
  ...
  const { serialized, serializedBytes } = serializeState(state, {
    stateFile,
    enforceEntryLimit: !allowEntryLimitMigration,
    enforceByteLimit: !allowEntryLimitMigration,
  });
  ...
  try {
    ...
    // The target remains untouched until this one atomic replacement. There
    // is no backup-first gap, and no fallible permission or identity work is
    // performed after commit. Keeping the verified descriptor open binds the
    // committed bytes to the object checked above.
