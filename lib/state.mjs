...
function validateState(
  ...
  for (const [key, entry] of Object.entries(state)) {
    ...
    // shape. Check the discriminator first so ordinary GitHub state entries do
    // not pay an Object.keys() scan during validation/GC.
    if (
      isPlainObject(entry) &&
...
export function expireReviewState(state, nowMs = Date.now()) {
  ...
  for (const [key, entry] of Object.entries(state)) {
    ...
    if (
      isPlainObject(entry) &&
      Object.hasOwn(entry, 'bitbucketTerminalHeadSha') &&
      validBitbucketTerminalHead(entry)
    ) continue;
