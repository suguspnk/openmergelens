...
function canonicalizeState(state) {
  ...
  const legacyProofScope = state[STATE_METADATA_KEY]?.reviewStateProofAfterScope;
  const legacyProofKey = isCanonicalReviewScope(legacyProofScope)
    ? state[STATE_METADATA_KEY]?.reviewStateProofAfterKeys?.[legacyProofScope]
    : null;
  if (typeof legacyProofKey === 'string') {
    reorderReviewStateProofQueue(normalized, legacyProofScope, legacyProofKey);
  }
...
export async function loadState(stateFile, { hardenPermissions = true } = {}) {
  try {
    ...
    const { normalizedState: state } = serializeState(parsedState, {
      stateFile,
    });
