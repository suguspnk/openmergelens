  ...
  for (let attempt = 0; attempt < WINDOWS_RETENTION_LOCK_RETRY_LIMIT; attempt += 1) {
    try {
      ...
      try {
        ...
        const contents = await probe.readFile({ encoding: 'utf8' });
        if (contents === WINDOWS_RETENTION_LOCK_BLOCKED) {
        ...
        if (probeError.code === WINDOWS_RETENTION_CAPACITY_ERROR) throw probeError;
...
async function openWindowsTemporary(
  ...
  return withWindowsRetentionQueue(parentPath, async () => {
    ...
    try {
      ...
      // Keep one deterministic parent-scoped marker for a rejected
      // reservation. Never delete or generate a UUID-suffixed evidence file:
      // the open handle remains bound to this inode while the pathname is
      // retained, and later callers recognize the marker and fail quickly.
      try {
        await lockHandle.truncate(0);
        await lockHandle.write(WINDOWS_RETENTION_LOCK_BLOCKED, 0, 'utf8');
        await lockHandle.sync();
      } catch {
        // An empty retained lock still fails closed; do not attempt unsafe
        // pathname cleanup or replacement of the marker.
      }
