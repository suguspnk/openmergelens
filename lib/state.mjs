...
function cloneStateMetadata(metadata) {
  ...
  if (metadata.reviewStateProofAfterScope !== undefined) {
    clone.reviewStateProofAfterScope = metadata.reviewStateProofAfterScope;
  }
  if (isPlainObject(metadata.reviewStateProofAfterKeys)) {
    clone.reviewStateProofAfterKeys = Object.fromEntries(
      Object.entries(metadata.reviewStateProofAfterKeys),
    );
  }
  return clone;
}
...
export function recordCandidateCursor(state, cursorKey, offset) {
  ...
  if (current?.reviewStateProofAfterScope !== undefined) {
    state[STATE_METADATA_KEY].reviewStateProofAfterScope =
      current.reviewStateProofAfterScope;
  }
  if (isPlainObject(current?.reviewStateProofAfterKeys)) {
    state[STATE_METADATA_KEY].reviewStateProofAfterKeys = Object.fromEntries(
      Object.entries(current.reviewStateProofAfterKeys),
    );
  }
}
...
export function reviewStateProofAfterScope(state) {
  const scope = state?.[STATE_METADATA_KEY]?.reviewStateProofAfterScope;
  return isCanonicalReviewScope(scope) ? scope : null;
}

export function recordReviewStateProofAfterScope(state, scope) {
  if (!isCanonicalReviewScope(scope)) {
    throw new Error('review-state proof scope cursor is invalid');
  }
  const current = state[STATE_METADATA_KEY];
  state[STATE_METADATA_KEY] = {
    version: STATE_METADATA_VERSION,
  ...
  if (current?.reviewStateGcAfterKey !== undefined) {
    state[STATE_METADATA_KEY].reviewStateGcAfterKey = current.reviewStateGcAfterKey;
  }
  if (isPlainObject(current?.reviewStateProofAfterKeys)) {
    state[STATE_METADATA_KEY].reviewStateProofAfterKeys = Object.fromEntries(
      Object.entries(current.reviewStateProofAfterKeys),
...
export function reviewStateProofAfterKey(state, scope) {
  const key = state?.[STATE_METADATA_KEY]?.reviewStateProofAfterKeys?.[scope];
  return typeof key === 'string' && reviewScopeKey(key) === scope ? key : null;
}

export function recordReviewStateProofAfterKey(state, scope, key) {
  if (
    !isCanonicalReviewScope(scope) ||
    typeof key !== 'string' ||
    !key ||
    ...
    throw new Error('review-state proof cursor is invalid');
  }
  const current = state[STATE_METADATA_KEY];
  const proofCursors = Object.fromEntries(
    isPlainObject(current?.reviewStateProofAfterKeys)
      ? Object.entries(current.reviewStateProofAfterKeys)
      : [],
  );
  if (!Object.prototype.hasOwnProperty.call(proofCursors, scope) &&
      Object.keys(proofCursors).length >= MAX_CANDIDATE_CURSOR_ENTRIES) {
    throw new Error('review-state proof cursor limit reached');
  }
  proofCursors[scope] = key;
  state[STATE_METADATA_KEY] = {
    version: STATE_METADATA_VERSION,
    candidateCursors: Object.fromEntries(
      isPlainObject(current?.candidateCursors)
    ...
    reviewStateProofAfterKeys: proofCursors,
  };
  if (current?.reviewStateGcAfterKey !== undefined) {
    state[STATE_METADATA_KEY].reviewStateGcAfterKey = current.reviewStateGcAfterKey;
  }
