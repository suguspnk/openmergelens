...
export async function saveState(
  ...
  const backupPath = `${stateFile}.bak-${process.pid}-${randomUUID()}`;
  let committed = false;
  let handle;
  let parentHandle;
  ...
  try {
    ...
    const [parentHandleStats, parentPathStats] = await Promise.all([
      parentHandle.stat(),
      lstat(parentPath),
    ]);
    if (
      !parentHandleStats.isDirectory() ||
      !parentPathStats.isDirectory() ||
      parentHandleStats.dev !== parentPathStats.dev ||
      parentHandleStats.ino !== parentPathStats.ino
    ) {
      throw new Error('review state parent directory identity changed before commit');
    }
    if (
      process.platform !== 'win32' &&
      (
        parentHandleStats.uid !== process.getuid() ||
        (parentHandleStats.mode & 0o077) !== 0
      )
    ) {
      throw new Error('review state parent directory must be private and user-owned');
    }
    ...
    try {
      ...
      if (!targetStats.isFile()) {
        throw new Error('existing review state target is not a regular file');
      }
      await rename(stateFile, backupPath);
      targetBackedUp = true;
    } catch (err) {
      if (err.code !== 'ENOENT') throw err;
    }
    // Tests can deterministically substitute the verified path here. The held
    // file handle stays open across rename, so the post-rename identity check
    // rejects that substitution and restores the prior target from its backup.
    await afterIdentityCheck(temporaryPath);
    await rename(temporaryPath, stateFile);
    temporaryMoved = true;
    const committedStats = await lstat(stateFile);
    if (
      !committedStats.isFile() ||
      handleStats.dev !== committedStats.dev ||
      handleStats.ino !== committedStats.ino
    ) {
      throw new Error('review state temporary file identity changed during commit');
    }
    await handle.close();
    handle = undefined;
    committed = true;
    if (targetBackedUp) {
      try {
        ...
        // The new target is already durably committed. A retained private
        // backup is safer than reporting rollback after the commit boundary.
      }
    }
  } catch (err) {
    if (temporaryMoved) await rm(stateFile, { force: true });
    if (targetBackedUp) {
      await rename(backupPath, stateFile);
      targetBackedUp = false;
    }
    throw err;
  } finally {
    ...
    if (!committed) await rm(backupPath, { force: true });
  }
}
