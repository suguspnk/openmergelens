...
export async function loadState(
  ...
  try {
    ...
    if (!samePathIdentity(pathStats, pathProofStats, { platform })) {
...
// they refer to the same object. Path-to-path checks still use the stable
// volume identity whenever it is available, so capture and compare it
// independently of the handle/path inode binding. If Windows reports no
// volume identifier, the file index is the only remaining stable proof.
export function samePathIdentity(
  ...
  const [leftIdentity, rightIdentity] = identities;
  if (platform === 'win32' &&
      !leftIdentity.dev.available && !rightIdentity.dev.available) {
    // Some Windows filesystems report no volume identifier. In that case the
    // file index is the only stable path identity available; do not coerce
    // zero/missing dev values into a cross-volume claim.
    return leftIdentity.ino.value === rightIdentity.ino.value;
  }
  if (!leftIdentity.dev.available || !rightIdentity.dev.available) return false;
...
async function verifyPrivateParent(
  ...
  if (
    ...
    !samePathIdentity(pathStats, pathProofStats, { platform })
  ) {
...
async function verifyPathIdentity(
  ...
  if (
    ...
    !samePathIdentity(currentStats, currentPathProof, { platform }) ||
