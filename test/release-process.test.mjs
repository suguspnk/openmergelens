...
test('release metadata is finalized as 1.7.0', async () => {
  const packageJson = JSON.parse(await readProjectFile('package.json'));
  const changelog = await readProjectFile('CHANGELOG.md');

  assert.equal(packageJson.version, '1.7.0');
  assert.match(changelog, /^## \[1\.7\.0] - \d{4}-\d{2}-\d{2}$/m);
  assert.match(changelog, /^## \[1\.6\.0] - \d{4}-\d{2}-\d{2}$/m);
  assert.match(changelog, /^## \[1\.5\.2] - \d{4}-\d{2}-\d{2}$/m);
